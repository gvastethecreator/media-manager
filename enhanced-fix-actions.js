	...
	// 3. Corregir funciones sin tipo de retorno
	{
		...
		replacement: (match, fnName, brace, offset, string, filePath) => {
			...
			if (entity) {
				...
				} else if (fnName.startsWith('create')) {
...
${match}`;
	...
	// 8. Corregir tipos en exportacion de funciones CRUD
	{
		...
		replacement: (match, actionType, entityName, params, offset, string, filePath) => {
			const entity = inferEntityFromEntityName(entityName) || inferEntityFromPath(filePath);
			if (!entity) return match;

			// Corregir tipos de retorno basado en la acción
			const returnType =
				actionType === 'get'
					? `Promise<${entity}Extended | null>`
					: actionType === 'delete'
						? 'Promise<void>'
						: actionType === 'getAll' || actionType === 'list'
...
function inferEntityFromPath(filePath) {
	...
	const match = filePathString.match(pathRegex);
	...
	if (match) {
		...
		if (entityDir.includes('-')) {
			...
			const camelCased = parts.map((part, i) => part.charAt(0).toUpperCase() + part.slice(1)).join('');
	...
	const fileMatch = filePathString.match(/[/\\]([\w-]+)(?:-crud|-query|-actions)?\.actions\.ts$/);
	if (fileMatch) {
...
async function main() {
	...
	fs.writeFileSync(
		RESULTS_FILE,
		`# Log de correcciones de server actions\n\nIniciado: ${new Date().toISOString()}\n\n`
	);
