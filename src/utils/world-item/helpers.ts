...
export function calculatePowerLevel(stats: WorldItemStats): number {
	...
	if (stats.effects && stats.effects.length > 0) {
		powerLevel += stats.effects.length * 5;
		statCount++;
	}
...
export function meetsRequirements(
	...
	requiredRarities?: RarityLevel[]
): boolean {
	// Verificar nivel
	if (requiredLevel !== undefined && worldItem.statsObject.level) {
		if (worldItem.statsObject.level < requiredLevel) {
			return false;
		}
	}
	...
	if (requiredRarities && requiredRarities.length > 0) {
		if (!requiredRarities.includes(worldItem.rarity as RarityLevel)) {
			return false;
		}
	}
...
export function filterWorldItems(
	...
	return worldItems.filter((item) => {
		...
		// Filtrar por nivel
		if (filters.minLevel !== undefined && item.statsObject.level !== undefined) {
			if (item.statsObject.level < filters.minLevel) {
				return false;
			}
		}

		if (filters.maxLevel !== undefined && item.statsObject.level !== undefined) {
			if (item.statsObject.level > filters.maxLevel) {
				return false;
			}
		}

		// Filtrar por valor
		if (filters.minValue !== undefined && item.statsObject.value !== undefined) {
			if (item.statsObject.value < filters.minValue) {
				return false;
			}
		}

		if (filters.maxValue !== undefined && item.statsObject.value !== undefined) {
			if (item.statsObject.value > filters.maxValue) {
				return false;
			}
		}
