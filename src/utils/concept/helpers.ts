  ...
  try {
    ...
    if (bestMatches.length < maxResults && conceptCategory) {
      const sameCategoryNotIncluded = otherConcepts
        .filter(c =>
          c.category === conceptCategory &&
          !bestMatches.some(m => m.concept.id === c.id)
        )
    ...
    return bestMatches.map(match => ({
      ...
      previewContent: match.concept.content
        ? `${match.concept.content.substring(0, 100)}...`
        : undefined,
      lastUpdated: match.concept.updatedAt instanceof Date
        ? match.concept.updatedAt
        : new Date(match.concept.updatedAt),
    }));
  } catch (error) {
