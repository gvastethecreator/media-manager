...
function DataGridColumnHeader<TData, TValue>({
	...
	const headerLabel = () => {
		return (
			<div
				className={cn(
					'inline-flex h-full items-center gap-1.5 font-normal text-[0.8125rem] text-accent-foreground leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60',
					className
				)}
			>
				{icon && icon}
	...
	const headerButton = () => {
		return (
			<Button
				className={cn(
					'-ms-2 h-7 rounded-md px-2 font-normal text-secondary-foreground hover:bg-secondary hover:text-foreground data-[state=open]:bg-secondary data-[state=open]:text-foreground',
					className
				)}
						...
						{column.getCanSort() && (
							<>
								...
								>
									...
									{column.getIsSorted() === 'asc' && <Check className="size-4 text-primary opacity-100!" />}
								</DropdownMenuItem>
								<DropdownMenuItem
									disabled={!column.getCanSort()}
									onClick={() => {
								...
								>
									...
									{column.getIsSorted() === 'desc' && <Check className="size-4 text-primary opacity-100!" />}
								</DropdownMenuItem>
							</>
						)}
