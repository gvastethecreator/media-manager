...
const navigationMenuTriggerStyle = cva(
	'group inline-flex h-9 w-max cursor-pointer items-center justify-center rounded-md bg-background px-4 py-2 font-medium text-sm outline-ring/50 ring-ring/10 transition-[color,box-shadow] hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus-visible:outline-1 focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50 data-[active=true]:bg-accent/50 data-[state=open]:bg-accent/50 data-[active=true]:text-accent-foreground dark:outline-ring/40 dark:ring-ring/20'
);
	...
	return (
		<div className={cn('absolute top-full left-0 isolate z-50 flex justify-center')}>
			<NavigationMenuPrimitive.Viewport
				className={cn(
					'data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full origin-top-center overflow-hidden rounded-md border border-border bg-popover p-1.5 text-popover-foreground shadow-black/5 shadow-md data-[state=closed]:animate-out data-[state=open]:animate-in md:w-[var(--radix-navigation-menu-viewport-width)]',
					className
				)}
...
function NavigationMenuLink({ className, ...props }: React.ComponentProps<typeof NavigationMenuPrimitive.Link>) {
	return (
		<NavigationMenuPrimitive.Link
			className={cn(
				'flex flex-col gap-1 rounded-md p-2 text-sm outline-ring/50 ring-ring/10 transition-[color,box-shadow] hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus-visible:outline-1 focus-visible:ring-4 data-[active=true]:bg-accent/50 data-[active=true]:text-accent-foreground dark:outline-ring/40 dark:ring-ring/20',
				className
			)}
...
function NavigationMenuIndicator({
	...
	return (
		<NavigationMenuPrimitive.Indicator
			...
			{...props}
		>
			<div className="relative top-[60%] h-2 w-2 rotate-45 rounded-ts-md bg-border shadow-md" />
		</NavigationMenuPrimitive.Indicator>
	);
}
