...
import { motion, type SpringOptions, type UseInViewOptions, useInView } from 'motion/react';
...
function GithubButton({
	...
	// Reset animation state when initialStars changes (targetStars no es necesario como dependencia)
	useEffect(() => {
		...
		>
			...
			{showStarIcon && (
				<div className="relative inline-flex shrink-0">
					<Star aria-hidden="true" className="fill-muted-foreground text-muted-foreground" />
					<Star
						aria-hidden="true"
			...
			<div className={cn('relative flex flex-col overflow-hidden font-semibold', starsClass)}>
				<motion.div
					animate={{ opacity: 1 }}
				...
				{fixedWidth && <span className="h-0 overflow-hidden tabular-nums opacity-0">{formatNumber(targetStars)}</span>}
			</div>
		</button>
	);
}
