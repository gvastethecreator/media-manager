        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {mockImageIds.map((id, index) => (
            <ImageCardImproved
              key={id}
              imageId={id}
              variant={index % 5 === 0 ? 'default' :
                      index % 5 === 1 ? 'minimal' :
                      index % 5 === 2 ? 'polaroid' :
                      index % 5 === 3 ? 'tcg' : 'gallery'}
              isSelected={index === 2}
            />
          ))}
        </div>
