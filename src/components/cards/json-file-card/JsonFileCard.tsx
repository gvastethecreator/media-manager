...
export function JsonFileCard({
	...
	const secondaryColor = useMemo(() => darkenHex(primaryColor, 0.3), [primaryColor]);
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-2 border-border/50 border-b p-3">
				<FileJsonIcon className="h-4 w-4" style={{ color: primaryColor }} />
				<div className="flex-1">
					<h3 className="truncate font-medium text-sm">{jsonFile.name}</h3>
					<p className="text-muted-foreground text-xs">
						{formatBytes(jsonFile.size)} • {jsonStats.keys} claves
					</p>
				</div>
				{jsonFile.isFavorite && <span className="text-yellow-500">⭐</span>}
			</div>
