...
import { getRecentPromptImages } from './prompt-server-actions';
...
/**
 * Componente para mostrar las imágenes recientes de un prompt en una tarjeta.
 * Similar a la sección de ilustración de una carta Magic.
 */
export function PromptCardImages({ promptId, primaryColor, secondaryColor }: PromptCardImagesProps) {
	...
	useEffect(() => {
		const loadImages = async () => {
			try {
				setIsLoading(true);
				const data = await getRecentPromptImages(promptId);
