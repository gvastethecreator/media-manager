...
const worldItemLogger = logger.withContext('WorldItemsSettings');
...
export function WorldItemsSettings() {
	...
	const stats = React.useMemo(() => {
		...
		const totalImages = worldItems.reduce((acc, item) => acc + (item._count?.images || 0), 0);
		const totalSize = worldItems.reduce((acc, item) => acc + (item.totalSize || 0), 0);
	...
	const handleUpdate = async (data: WorldItemFormData) => {
		...
		try {
			...
			toastService.success('El objeto del mundo se ha actualizado correctamente.');
			setEditingId(null);
		} catch (error) {
	...
	const handleDelete = async (id: string) => {
		try {
			...
			toastService.success('El objeto del mundo se ha eliminado correctamente.');
		} catch (error) {
			...
			<Card className="rounded-sm bg-muted/30">
				<CardHeader className="p-3">
					<CardTitle className="flex items-center justify-between text-sm">
						...
						<Button variant="outline" size="sm" onClick={() => loadWorldItems()} disabled={isLoading}>
				...
				<CardContent>
					...
					{worldItems.length === 0 && !isLoading ? (
						<div className="text-sm text-muted-foreground text-center py-4">No hay objetos del mundo creados</div>
					) : (
