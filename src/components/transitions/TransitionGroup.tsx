...
import { getEnterExitCoordinator, type TransitionDirection } from '@/lib/transitions';
...
interface TransitionGroupContextValue {
  groupId: string;
  registerElement: (id: string, index: number) => void;
  ...
  return (
    <TransitionGroupContext.Provider value={contextValue}>
      <div 
        className={cn(
          'transition-group',
          isAnimating && 'transition-group-animating',
          className
        )}
...
export const TransitionItem = React.memo(function TransitionItem({
  ...
  useEffect(() => {
    ...
    if (element) {
      context.registerElement(id, indexRef.current)(element);
    }
...
export function AnimatePresence({
  ...
  useEffect(() => {
    if (present && !shouldRender) {
      ...
      if (elementRef.current) {
        coordinator.current.coordinateExit([{
          id: 'animate-presence',
          element: elementRef.current,
          index: 0,
        }], exit).then(() => {
  ...
  return (
    <div 
      ...
      className={cn(
        'animate-presence',
        isExiting && 'animate-presence-exiting',
        className
      )}
    >
      {children}
