...
import { getMorphEngine, generateClipPath, generateBorderRadius } from '@/lib/transitions';
...
export const MorphContainer = React.memo(function MorphContainer({
  ...
  return (
    <div
      ...
      className={cn(
        'morph-container',
        isMorphing && 'morph-transitioning',
        `morph-shape-${shape}`,
        className
      )}
...
export const LiquidContainer = React.memo(function LiquidContainer({
  ...
  useEffect(() => {
    ...
    const startAnimation = async () => {
      ...
      animationRef.current = createLiquidMorph(
        containerRef.current!,
        intensity,
        duration
      );
