...
import { DragSelectionManager, DragSelectionConfig, DragSelectionState } from '../../../../services/drag-selection/drag-selection-manager';
...
export const DragSelectionProvider: React.FC<DragSelectionProviderProps> = ({
  ...
  const toggleItems = useCallback((ids: string[]) => {
    ids.forEach(id => {
      const item = items.find(item => item.id === id);
