...
const fetchStoreData = async (entity: SupportedEntities): Promise<any[]> => {
	...
	if (!store || !loadMethod) {
		...
		try {
			...
			switch (entity) {
				case 'tags': {
					const { getTags } = await import('@/app/actions/tags/query.actions');
	...
	try {
		...
		loadingLogger.info(
			`✅ Datos de ${entity} cargados desde store: ${Array.isArray(data) ? data.length : 'N/A'} items`
		);
		return data || [];
	} catch (storeError) {
...
export const useEntityLoader = () => {
	...
	const handleOpenChange = useCallback(
		(open: boolean, entity: SupportedEntities) => {
