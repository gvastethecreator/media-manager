...
export function VirtualizerWrapper<T>({
	...
	// Referencia para la función de actualización de índices visibles
	const updateVisibleIndicesRef = useRef<() => void>(() => {
		// Implementación inicial vacía
	});

	// Función para llamar a la referencia actualizada
	const updateVisibleIndices = () => updateVisibleIndicesRef.current();
	...
	// Reemplazar la implementación de updateVisibleIndices con la versión real
	useEffect(() => {
		// Redefinir la función para evitar referencias cíclicas
		const newUpdateVisibleIndices = () => {
		...
		(updateVisibleIndices as any) = newUpdateVisibleIndices;
	}, [
		gridVirtualizer,
		listVirtualizer,
		type,
