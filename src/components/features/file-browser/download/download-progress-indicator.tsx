...
import { formatFileSize, formatDuration } from '@/lib/utils/format.utils';
...
export function DownloadProgressIndicator({
  ...
  const activeOperations = downloadOperations.filter(op => 
    ['pending', 'running'].includes(op.status)
  );
  const completedOperations = downloadOperations.filter(op => 
    op.status === 'completed'
  );
  const failedOperations = downloadOperations.filter(op => 
    op.status === 'failed'
  );
              ...
              <div className="flex-1 min-w-0">
                {activeOperations.length > 0 ? (
                  ...
                  <div className="text-sm font-medium text-green-600 dark:text-green-400">
                    {completedOperations.length} descarga{completedOperations.length > 1 ? 's' : ''} completada{completedOperations.length > 1 ? 's' : ''}
                  </div>
                )}
              </div>
        ...
        {isExpanded && (
          ...
          >
            ...
            <div className="max-h-64 overflow-y-auto">
                ...
                <div key={operation.id} className="p-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {operation.items.processed} de {operation.items.total} archivo{operation.items.total > 1 ? 's' : ''}
                  ...
                  <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {operation.progress.speed && (
                      <span>{operation.progress.speed.toFixed(1)} archivos/s</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              {/* Completed Downloads */}
              {showCompleted && completedOperations.map((operation) => (
                <div key={operation.id} className="p-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0 bg-green-50 dark:bg-green-900/10">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-green-600 dark:text-green-400">
                        {operation.items.total} archivo{operation.items.total > 1 ? 's' : ''} descargado{operation.items.total > 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {failedOperations.map((operation) => (
                <div key={operation.id} className="p-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0 bg-red-50 dark:bg-red-900/10">
