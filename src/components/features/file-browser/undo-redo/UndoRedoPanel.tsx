			...
			<CardHeader className={cn('pb-3', compact && 'pb-2')}>
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						{history.length > 0 && (
							<Button variant="outline" size="sm" onClick={handleClear} disabled={isClearing} className="text-xs">
								Limpiar
							</Button>
						)}
						{onClose && (
							...
							{history.map((action, index) => {
								...
								return (
									<div key={action.id}>
										...
										>
											<div className={cn('flex items-center gap-2 flex-1 min-w-0', compact && 'gap-1.5')}>
												<Icon className={cn('h-4 w-4 flex-shrink-0', compact && 'h-3 w-3')} />

												<div className="flex-1 min-w-0">
													<p className={cn('text-sm font-medium truncate', compact && 'text-xs')}>
														{getActionDescription(action)}
													</p>

													{showDetails && action.metadata?.targetPath && (
														<p className={cn('text-xs text-muted-foreground truncate', compact && 'text-[10px]')}>
											...
											<div className="flex items-center gap-2 flex-shrink-0">
												...
												{isCurrent && (
													<Badge
														variant="secondary"
														className={cn('text-xs px-1.5 py-0.5', compact && 'text-[10px] px-1')}
													>
														Actual
													</Badge>
												)}
