...
export function EntityPreloader({
  ...
  useEffect(() => {
    ...
    if (timeSinceLastMount < 100 && preloaderInstanceCount > 1) {
      preloaderLogger.warn(`⚠️ Posible ciclo de montaje detectado (${preloaderInstanceCount} instancias, ${timeSinceLastMount}ms)`);
    }
  ...
  useEffect(() => {
    ...
    preloaderLogger.info(`🚀 Iniciando precarga de ${entitiesToPreload.length} entidades (modo: ${mode}, instancia: ${instanceIdRef.current})`);
    ...
    const preloadEntities = async () => {
      try {
        ...
        const criticalEntities = entitiesToPreload.filter(e =>
          e === 'collections' || e === 'tags' || e === 'albums'
        );
        const nonCriticalEntities = entitiesToPreload.filter(e =>
          e !== 'collections' && e !== 'tags' && e !== 'albums'
        );
        ...
        for (const criticalEntity of criticalEntities) {
          ...
          try {
            await loadEntityData(criticalEntity as any)
              .catch(err => {
        ...
        if (!isUnmountingRef.current) {
          const nonCriticalPromises = nonCriticalEntities.map(entityName =>
            loadEntityData(entityName as any)
              .catch(err => {
        ...
        if ((mode === 'all' || !respectGlobalState) && typeof window !== 'undefined') {
          window.entityPreloadComplete = true;
          window.entityPreloadInProgress = false;
          preloaderLogger.info(`🏁 Precarga global marcada como completada a pesar de errores (${instanceIdRef.current})`);
        }
