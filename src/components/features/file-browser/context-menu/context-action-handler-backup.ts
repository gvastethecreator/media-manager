...
import { getEntitySize, getEntityPath } from '@/lib/utils/entity-properties.utils';
...
const customFileOperationsService = {
	...
	renameFile: async (oldPath: string, newName: string) => {
		try {
			...
			// Use fallback to original service for now
			await renameFileService(oldPath, newName);
			actionLogger.info('✅ Archivo renombrado:', { oldPath, newName });
...
function convertFileItemToEntity(item: FileItem): AnyEntityWithStats {
	...
	let entityType: string = 'image'; // Default
	let entityData: any = {};
	...
	if ('mimeType' in item) {
		...
		if (mimeType.startsWith('image/')) {
			...
			entityData = {
				...entityData,
				path: getEntityPath(item as any) || '',
				hash: '',
				size: getEntitySize(item as any),
				width: 0,
				height: 0,
				metadata: null,
				thumbnail: null,
	...
	// Create the base entity with required fields
	const baseEntity = {
		id: item.id,
		name: item.name,
