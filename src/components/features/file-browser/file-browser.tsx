...
const FileBrowserComponent = ({
	...
	useEffect(() => {
		...
		if (typeof window !== 'undefined' && window.entityPreloadInProgress) {
			...
			const now = Date.now();
			if (now - preloadStartTime > 5000) { // 5 segundos
		...
		(fileItem: FileItem): any => {
			try {
				...
				if ((!width || !height) && processedItem.metadata) {
					try {
						const metadataObj = typeof processedItem.metadata === 'string'
							? JSON.parse(processedItem.metadata)
				...
				// ✨ Log para depuración
				gridLogger.debug(`Mapped item: ${processedItem.id}, finalSrc: ${thumbnailUrl}, dimensions: ${width || 0}x${height || 0}`);
				...
				return {
					...
					metadata: typeof processedItem.metadata === 'string' ? processedItem.metadata : JSON.stringify(processedItem.metadata || {}),
