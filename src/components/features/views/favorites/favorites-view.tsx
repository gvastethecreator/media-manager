...
async function fetchFavorites(pageIndex: number) {
  const response = await fetch(`/api/favorites?page=${pageIndex}&pageSize=${PAGE_SIZE}`);
  if (!response.ok) throw new Error('Network response was not ok');
  return response.json();
}

export function FavoritesView({ isResizing }: ViewProps) {
  ...
  if (isFetching && !isFetchingNextPage && allRows.length === 0) {
    return (
  ...
  if (!isFetching && allRows.length === 0) {
    return (
      <EmptyState
        ...
        description="Las imágenes que marques como favoritas aparecerán aquí"
        className="h-full"
      />
    );
  }
