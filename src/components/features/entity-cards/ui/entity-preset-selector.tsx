...
export function EntityPresetSelector({
	...
	const handleApplyPreset = useCallback(async (presetId: string | null) => {
		try {
			setIsLoading(true);
			...
			if (response.success) {
				...
				toast({
					...
					description: presetId
						? 'Preset visual aplicado correctamente'
						: 'Preset visual removido correctamente',
				});
			} else {
				toast({
					title: 'Error',
	...
	const onPresetSelect = useCallback((presetId: string) => {
	...
	return (
		<div className={cn('flex flex-col space-y-2', className)}>
			<div className="flex items-center justify-between">
				<label htmlFor="preset-selector" className="text-sm font-medium">Preset Visual</label>
				{selectedPresetId && (
					<Button
						variant="ghost"
						size="sm"
