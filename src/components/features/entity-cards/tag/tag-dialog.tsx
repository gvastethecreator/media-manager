	...
	const handleSave = async (imageId?: string | null) => {
		...
		try {
			...
			if (imageId && savedTag) {
				...
				toastService.success(`Se ha añadido la etiqueta "${formData.name}" a la imagen`);
			}
		...
		>
			<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
				...
				<div className="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-4">
					<TagForm initialData={formData} onSubmit={handleFormSubmit} onCancel={handleCancel} isLoading={isLoading} />
				</div>
				...
				<div className="flex flex-col space-y-4">
					...
					<p className="text-xs text-muted-foreground">
						Esta es una previsualización de la etiqueta. Los campos opcionales se mostrarán solo si contienen
						información.
					</p>
				</div>
			</div>
