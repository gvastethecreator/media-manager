...
export function PlaceCard({ data, isPreview = false, onEdit, onDelete, onClick, className }: PlaceCardProps) {
	...
	const _totalSize = 'totalSize' in data && data.totalSize ? formatBytes(data.totalSize) : null;

	// Obtener iconos para clima y tipo
	const climateIcon = 'climate' in data && data.climate ? getClimateIcon(data.climate) : null;
				...
				<div className={cn('relative h-36 bg-gradient-to-br', gradient)}>
					...
					{hasFeaturedImage && (
						<div className="absolute inset-0">
							<Image src={data.featuredImage as string} alt={data.name} fill className="object-cover opacity-50" />
					...
					<div className="absolute bottom-0 left-0 right-0 p-3 text-white">
						<div className="flex items-center gap-2">
							...
							<div>
								...
								{'region' in data && data.region && <div className="text-xs opacity-90">Región: {data.region}</div>}
							</div>
						</div>
					</div>
