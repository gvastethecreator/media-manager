...
function getWorldItemPower(worldItem: WorldItem) {
	...
	const properties = worldItem.properties
		? JSON.parse(worldItem.properties as string)
		: [];
	...
	return Math.max(
		1,
		Math.min(12, baseValue + propertyBonus + statsBonus + imageBonus)
	);
}
				...
				rarity={{
					...
					glowColor:
						rarity.label === "Legendario"
							? rarity.barClass.split("from-")[1].split(" ")[0]
							: undefined,
				}}
			...
			>
				...
				<div
					className={cn(
						...
						typeof cardOptions.contentPadding === "number" &&
							`p-${cardOptions.contentPadding}`,
						typeof cardOptions.contentSpacing === "number" &&
							`gap-${cardOptions.contentSpacing}`
					)}
				>
					...
					<div className="absolute top-3 left-3 w-12 h-12 rounded-full bg-background/90 backdrop-blur-sm border-2 flex items-center justify-center z-10 shadow-lg overflow-hidden">
						<div
							className={cn("absolute inset-0 opacity-70", rarity.badgeClass)}
						/>
					...
					<div className="relative px-3 py-2 bg-background/80 backdrop-blur-md shadow-sm border-b border-border z-10">
						...
						<div className="text-xs font-medium text-muted-foreground flex items-center mt-0.5">
							...
							{worldItem.type && (
								<span className="ml-1">• {worldItem.type}</span>
							)}
						</div>
					</div>
						...
						{cardOptions.useImageGrid ? (
							<ImageGrid
								...
								images={[
									{
										id: "world-item-image",
										path: worldItem.featuredImage || "",
										thumbnail: worldItem.featuredImage || "",
									},
								]}
							/>
						) : (
					...
					<div className="bg-background/90 backdrop-blur-md p-3 flex flex-col gap-2">
						...
						<div className="grid grid-cols-3 gap-3">
							...
							<div className="flex flex-col items-center">
								<div className="text-2xl font-bold">
									{worldItem.origin?.substring(0, 4) || "-"}
								</div>
								<div className="text-[10px] uppercase tracking-wide text-muted-foreground flex items-center gap-1">
