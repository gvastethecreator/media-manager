			...
			>
					...
					<div className="absolute bottom-0 left-0 right-0 p-3 text-white">
						<div className="flex items-center gap-2">
							<div className="flex items-center justify-center w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm">
								{('emoji' in data && data.emoji) || <FolderIcon className="w-4 h-4" />}
							</div>
							<h3 className="text-lg font-bold truncate">{'name' in data && data.name}</h3>
						</div>
					</div>
				...
				<div className="p-3 bg-white">
					...
					{'description' in data && data.description && (
						<p className="text-sm text-gray-600 line-clamp-2 mb-2">{data.description}</p>
					)}
