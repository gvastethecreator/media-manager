...
export function ImageGrid({
	...
	const imagesToShow = layout === 'single' ? 1 : layout === 'dual' ? 2 : layout === 'quad' ? 4 : 6;
	...
	const renderImage = (image: ImageGridImage | null, index: number) => {
		...
		const isThumbnailBase64 =
			image.thumbnail && (image.thumbnail.startsWith('data:') || image.thumbnail.startsWith('blob:'));
	...
	return (
		<div
			className={cn('grid h-full w-full', containerClasses[layout], gap > 0 && `gap-${gap}`, className)}
			style={{ aspectRatio }}
		>
