...
export const RarityBadge: React.FC<RarityBadgeProps> = ({
	...
	return (
		<div
			className={cn('inline-flex items-center rounded-full font-medium', sizeClasses[size], className)}
			style={{
...
export const RaritySelector: React.FC<RaritySelectorProps> = ({
	...
	return (
		<div className="space-y-2">
			...
			<div className={cn(displayType === 'grid' ? 'grid grid-cols-2 gap-2' : 'flex flex-col space-y-2')}>
				{rarities.map((rarity) => (
					<Button
						key={rarity.id}
						type="button"
						...
						className={cn(
							...
							selectedRarityId === rarity.id ? `bg-${rarity.color} text-white` : 'bg-transparent hover:bg-secondary/20'
						)}
						onClick={() => handleSelectRarity(rarity)}
						style={{
							borderColor: selectedRarityId === rarity.id ? rarity.borderColor : undefined,
							boxShadow:
								selectedRarityId === rarity.id ? `0 0 ${rarity.glowIntensity || 5}px ${rarity.glowColor}` : undefined,
						}}
					>
