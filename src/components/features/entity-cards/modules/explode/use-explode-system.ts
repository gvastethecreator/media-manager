...
import { ExplodeSystem } from './types';
...
export function useExplodeSystem(initialSystem?: Partial<ExplodeSystem>) {
   ...
   */
  const generateExplodeStyles = useCallback((layerIndex: number, totalLayers: number) => {
    ...
    return {
      ...
      ':hover': explodeSystem.expandOnHover
        ? { transform: transform.replace(/translateX\((\d+)px\)/, `translateX(${explodeSystem.distance * explodeSystem.hoverExpandFactor}px)`) }
        : undefined
    };
  }, [explodeSystem]);
