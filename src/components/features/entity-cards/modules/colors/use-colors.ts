...
export function useColors(props: UseColorsProps = {}) {
  ...
  const options = useMemo((): ColorsOptions => ({
   ...
   */
  const updateOption = useCallback((key: keyof ColorsOptions, value: unknown) => {
   ...
   */
  const toggleColorPalettes = useCallback((useColorPalettes: boolean) => {
