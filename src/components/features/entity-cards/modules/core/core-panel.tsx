...
export function CorePanel({
	...
	const { coreOptions, updateCoreOption } = useCoreSettings(options.core);

	// Manejar cambios en opciones específicas del núcleo
	const handleCoreChange = <K extends keyof typeof coreOptions>(key: K, value: (typeof coreOptions)[K]) => {
			...
			{coreOptions.enabled && (
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-6">
					...
					<TabsContent value="performance">
						<PerformanceSection coreOptions={coreOptions} handleCoreChange={handleCoreChange} disabled={disabled} />
					</TabsContent>

					<TabsContent value="feedback">
						<FeedbackSection coreOptions={coreOptions} handleCoreChange={handleCoreChange} disabled={disabled} />
					</TabsContent>

					<TabsContent value="content">
						<ContentSection coreOptions={coreOptions} handleCoreChange={handleCoreChange} disabled={disabled} />
					</TabsContent>
				</Tabs>
			)}
		</FormLayout>
