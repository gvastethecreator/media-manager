...
import { type CoreOptions, DEFAULT_CORE_OPTIONS } from '../types';
...
export function useCoreSettings(initialOptions?: Partial<CoreOptions>) {
	...
	const [coreOptions, setCoreOptions] = useState<CoreOptions>(merge({}, DEFAULT_CORE_OPTIONS, initialOptions || {}));
	 ...
	 */
	const updateCoreOption = useCallback(<K extends keyof CoreOptions>(key: K, value: CoreOptions[K]) => {
		setCoreOptions((prev) => ({
			...prev,
			[key]: value,
		}));
	 ...
	 */
	const updateLayerSystem = useCallback((key: string, value: unknown) => {
		setCoreOptions((prev) => {
