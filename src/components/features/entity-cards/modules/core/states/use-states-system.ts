...
export function useStatesSystem(props?: UseStatesSystemProps): UseStatesSystemResult {
	...
	// Actualiza un estado específico
	const updateState = useCallback(<K extends keyof StatesSystem>(stateName: K, stateConfig: StatesSystem[K]) => {
	...
	// Habilita o deshabilita un estado
	const toggleState = useCallback((stateName: keyof StatesSystem, enabled: boolean) => {
		if (enabled) {
	...
	const isStateEnabled = useCallback((stateName: keyof StatesSystem) => !!statesSystem[stateName], [statesSystem]);
