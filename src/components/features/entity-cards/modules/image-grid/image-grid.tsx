    ...
    >
      {displayImages.length > 0 ? (
        displayImages.map((image, index) => {
          ...
          const overlapStyles: React.CSSProperties = style === 'overlap' ? {
            top: `${index * 10}px`,
            left: `${index * 10}px`,
          ...
          const carouselStyles: React.CSSProperties = style === 'carousel' ? {
            minWidth: '85%',
        ...
        <div className="flex h-full w-full items-center justify-center bg-muted/30 min-h-[120px]" style={{ aspectRatio }}>
      ...
      {showCount && remainingCount > 0 && (
        <Badge
          className="absolute bottom-2 right-2 z-10 bg-background/80 backdrop-blur-sm"
          variant="outline"
        >
