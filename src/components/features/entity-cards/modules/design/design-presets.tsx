				...
				<div className="grid grid-cols-2 gap-4">
					...
					<Card className="overflow-hidden cursor-pointer hover:border-primary transition-colors">
						<CardContent className="p-0">
							...
							<div className="p-2">
								<DesignPreview designSystem={DEFAULT_DESIGN_SYSTEM} className="h-32" showPlaceholder />
							</div>
					...
					{/* Presets guardados */}
					{presets.map((preset) => (
						<Card key={preset.id} className="overflow-hidden cursor-pointer hover:border-primary transition-colors">
							<CardContent className="p-0">
								<div className="p-2 border-b">
								...
								<div className="p-2">
									<DesignPreview designSystem={preset.designSystem} className="h-32" showPlaceholder />
								</div>
								<div className="p-2 bg-muted/50 flex justify-between">
									<Button variant="ghost" size="sm" onClick={() => onDeletePreset(preset.id)}>
										Eliminar
									</Button>
									<Button variant="ghost" size="sm" onClick={() => onSelectPreset(preset)}>
										Aplicar
									</Button>
								</div>
							</CardContent>
