 ...
 */
export type GlowAnimationType = 'none' | 'pulse' | 'follow-mouse' | 'radial-pulse' | 'static';
...
const GlowLayerComponent = React.memo(function GlowLayerComponent({
	...
	const glowStyle = React.useMemo(
		() => ({
			...style,
			...generateGlowStyles(processedConfig.color, processedConfig.intensity, processedConfig.spread as number),
...
export function GlowSettings({
	...
	const handleChange = React.useCallback(
		(key: keyof GlowConfig) => (event: React.ChangeEvent<HTMLInputElement>) => {
			const value =
		...
		<div className="space-y-4">
			<div className="grid gap-4">
				<label className="flex flex-col gap-2">
					<span>Color</span>
					<input type="color" value={config.color} onChange={handleChange('color')} />
				</label>
				...
				<label className="flex flex-col gap-2">
					...
					<input type="range" min="0" max="50" value={config.spread || 0} onChange={handleChange('spread')} />
				</label>

				<label className="flex items-center gap-2">
					<input type="checkbox" checked={config.followMouse || false} onChange={handleChange('followMouse')} />
...
export const glowLayerImplementation: LayerImplementation<GlowConfig> = {
	...
	render: (props) => {
		...
		return (
			<GlowEffectLayer {...options} isHovered={isHovered} mousePosition={mousePosition} visible={visible}>
				{children}
			</GlowEffectLayer>
		);
	},
