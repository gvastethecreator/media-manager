...
interface UseDistortionEffectsResult {
  ...
  updateEffect: (effectName: 'glitchEffect' | 'chromaticAberration' | 'pixelate',
               config: Partial<DistortionEffectsSystem[keyof DistortionEffectsSystem]>) => void;
    ...
    (isHovered = false) => {
      ...
      if (effectsSystem.enabled) {
        ...
        if (
          ...
          (!effectsSystem.chromaticAberration.visibleOnHover || (effectsSystem.chromaticAberration.visibleOnHover && isHovered))
        ) {
