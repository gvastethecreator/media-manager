...
export function generateDistortionEffects(ctx: CanvasRenderingContext2D, options: DistortionEffectOptions): void {
	const { width, height, intensity = 0.5, time = 0, glitchEffect, chromaticAberration, pixelate } = options;
	...
	if (glitchEffect?.enabled) {
		generateGlitchEffect(ctx, width, height, glitchEffect.intensity, glitchEffect.frequency, time);
	}

	if (chromaticAberration?.enabled) {
		generateChromaticAberration(ctx, width, height, chromaticAberration.offset, chromaticAberration.intensity);
	}

	if (pixelate?.enabled) {
		generatePixelateEffect(ctx, width, height, pixelate.blockSize, pixelate.intensity);
	}
