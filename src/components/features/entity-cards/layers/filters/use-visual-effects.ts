...
export function useVisualEffects(props?: UseVisualEffectsProps): UseVisualEffectsResult {
	 ...
	 */
	const updateEffect = useCallback(<K extends keyof VisualEffectsOptions>(key: K, value: VisualEffectsOptions[K]) => {
		setEffects((prev) => ({
			...prev,
			[key]: value,
		}));
	...
	const generateCssFilters = useCallback(
		(_isHovering?: boolean): string => {
			const { brightness, contrast, saturate, hueRotate, grayscale, sepia, invert, opacity, blur, dropShadow } =
				effects;
			...
			if (saturate !== 100) {
				filters.push(`saturate(${saturate / 100})`);
			}
			if (hueRotate !== 0) {
			...
			if (invert !== 0) {
				filters.push(`invert(${invert / 100})`);
			}
			if (opacity !== 100) {
				filters.push(`opacity(${opacity / 100})`);
			}
			if (blur !== 0) {
				filters.push(`blur(${blur}px)`);
			}
			if (dropShadow) {
				filters.push('drop-shadow(0 8px 16px rgba(0, 0, 0, 0.25))');
			}
	 ...
	 */
	const generateBackdropCssFilters = useCallback(
		(_isHovering?: boolean): string => {
			const { backdropBlur, backdropBrightness, backdropSaturate, backdropOpacity } = effects;

			// Si no hay efectos de fondo activos, devuelve una cadena vacía
			if (backdropBlur === 0 && backdropBrightness === 100 && backdropSaturate === 100 && backdropOpacity === 100) {
				return '';
			}
			...
			if (backdropBlur !== 0) {
				filters.push(`blur(${backdropBlur}px)`);
			}
			if (backdropBrightness !== 100) {
				filters.push(`brightness(${backdropBrightness / 100})`);
			}
			if (backdropSaturate !== 100) {
				filters.push(`saturate(${backdropSaturate / 100})`);
			}
			if (backdropOpacity !== 100) {
				filters.push(`opacity(${backdropOpacity / 100})`);
			}
