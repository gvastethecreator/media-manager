...
export function PatternSelector({ patterns, selectedPatternId, color, onPatternSelect }: PatternSelectorProps) {
	return (
		<div className="space-y-2">
			...
			<div className="grid grid-cols-3 gap-2">
				{patterns.map((pattern) => (
					...
					>
						<div className="w-full h-full">{pattern.renderSvg(color || '#000000')}</div>
					</Button>
				))}
			</div>
		</div>
