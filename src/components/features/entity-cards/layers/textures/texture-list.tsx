	...
	return (
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				...
				<Button variant="outline" size="sm" onClick={onAddTexture} className="h-8 w-8 p-0">
					...
					{textures.map((texture) => (
						<button
							...
							className={`group relative w-full cursor-pointer rounded-lg border p-2 text-left transition-colors hover:bg-accent ${
								selectedTextureId === texture.id ? 'border-primary bg-accent' : 'border-transparent'
							}`}
							onClick={() => onSelectTexture(texture.id)}
						>
							<div className="flex items-center justify-between">
								<span className="text-sm font-medium">{texture.name || 'Textura sin nombre'}</span>
								<Button
									variant="ghost"
									size="icon"
