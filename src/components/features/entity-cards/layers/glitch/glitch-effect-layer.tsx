	...
	const applyGlitchAnimations = async () => {
		...
		const createGlitchKeyframes = () => {
			...
			for (let i = 0; i < steps; i++) {
				...
				const rgbShiftX = config.rgbShiftEnabled ? Math.random() * config.colorShiftAmount * 10 : 0;
				...
				keyframes.push({
					...
					boxShadow: config.rgbShiftEnabled
						? `rgba(255,0,0,${config.colorShiftAmount}) ${-rgbShiftX}px 0px,
						 ...
						 rgba(0,0,255,${config.colorShiftAmount}) ${rgbShiftX}px 0px`
						: 'none',
				});
			}
		...
		await animate(scope.current, createGlitchKeyframes(), {
			duration: config.duration,
			times: Array.from(
				{ length: Math.max(10, config.sliceCount * 2) },
				(_, i) => i / (Math.max(10, config.sliceCount * 2) - 1)
			),
			ease: 'linear',
		});
		...
		>
			...
			{(isActive || (config.visibleOnHover && isHovering)) && (
				<>
					...
					<motion.div
						ref={scope}
						className={cn('absolute inset-0 z-20 mix-blend-overlay', {
					...
					>
						...
						{config.affectContent && <div className="w-full h-full relative">{children}</div>}
					</motion.div>
