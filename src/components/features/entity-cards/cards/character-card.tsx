...
export function CharacterCard({ data, isPreview = false, onEdit, onDelete, onClick, className }: CharacterCardProps) {
				...
				<div className="z-10 flex flex-1 flex-col">
					<div className="flex items-center space-x-2">
						...
						<div>
							...
							{data && 'class' in data && data.class && <p className="text-sm text-muted-foreground">{data.class}</p>}
						</div>
	...
	const classGradient =
		data && 'class' in data ? getClassGradient(data.class) : 'from-slate-500/20 via-gray-500/20 to-slate-500/20';
			...
			<div className="relative z-10 h-full p-4 flex flex-col">
				...
				<div className="flex items-center gap-2">
					...
					<div className="flex-1 min-w-0">
						<h3 className="text-lg font-bold truncate text-white drop-shadow-lg">{data?.name || 'Sin nombre'}</h3>
				...
				<div className="mt-auto">
					{data && '_count' in data && data._count && (
						<div className="flex items-center justify-between text-xs text-white/60 mb-2">
							...
							{data && 'totalSize' in data && data.totalSize && <span>{formatBytes(data.totalSize)}</span>}
						</div>
					)}
