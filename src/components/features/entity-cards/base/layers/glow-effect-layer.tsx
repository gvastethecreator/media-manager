...
export function GlowEffectLayer({
	...
	React.useEffect(() => {
		if (mergedOptions.animationType !== "pulse") return;
		...
		const animate = () => {
			...
			const size =
				minSize + Math.abs(Math.sin(elapsed * Math.PI)) * (maxSize - minSize);
			setGlowSize(size);
		...
		mergedOptions.animationType,
		mergedOptions.size,
		mergedOptions.pulseSpeed,
	]);

	// Actualizar posición del brillo basado en el tipo de animación
	React.useEffect(() => {
		if (mergedOptions.animationType !== "follow-mouse") return;
	...
	return (
		<div
			...
			style={
				isExploded
					? getExplodeLayerTransform(mergedOptions.layerIndex || 4)
					: {}
			}
