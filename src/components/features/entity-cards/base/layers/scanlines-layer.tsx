...
import type { ExplodeLayerTransformFunction, ScanlinesOptions } from "../base-card-types";
...
export function ScanlinesLayer({
	...
	const getScanlinesGradient = () => {
		...
		switch (mergedOptions.direction) {
			case "vertical":
				return `repeating-linear-gradient(90deg, transparent, transparent ${spacing - 1}px, ${color} ${spacing}px)`;
			case "diagonal":
				return `repeating-linear-gradient(45deg, transparent, transparent ${spacing - 1}px, ${color} ${spacing}px)`;
			case "horizontal":
			default:
				return `repeating-linear-gradient(0deg, transparent, transparent ${spacing - 1}px, ${color} ${spacing}px)`;
		}
	};
