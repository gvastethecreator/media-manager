...
export function EntityStatsDisplay({
	stats,
	className,
	layout = "grid",
}: EntityStatsDisplayProps) {
	if (layout === "inline") {
		return (
			<div className={cn("flex items-center gap-3 text-xs", className)}>
				...
				{stats.customStats?.map((stat) => (
					<div
						key={`custom-stat-${stat.id || stat.label}`}
	...
	return (
		<div className={cn("grid grid-cols-3 gap-3", className)}>
			...
			{stats.customStats?.map((stat) => {
				...
				return (
					<div
						key={`stat-${stat.id || stat.label}`}
						className="flex flex-col items-center"
					>
						<div className="text-xl font-bold">{stat.value}</div>
						<div className="text-[10px] uppercase tracking-wide text-muted-foreground flex items-center gap-1">
							{React.isValidElement(stat.icon)
								? React.cloneElement(stat.icon, customIconProps)
								: stat.icon}
							{stat.label}
						</div>
					</div>
...
export function ThumbnailGrid({
	...
	return (
		<div
			className={cn(
				`grid grid-cols-${cols} grid-rows-${rows} gap-1 p-2 h-full`,
				className
			)}
		>
			{Array(rows * cols)
				...
				.map((_, i) => {
					...
					return (
						<button
							key={itemId}
							className={cn(
								"relative rounded overflow-hidden p-0 border-0",
								aspectRatioClass,
								itemClassName
							)}
...
export function FeaturedImage({
	...
	return (
		<button
			className={cn(
				"relative w-full h-full overflow-hidden p-0 border-0",
				className
			)}
			onClick={onClick}
			aria-label={alt}
