...
import { BookOpen, Info, Layers, LayoutTemplate, Sparkles } from 'lucide-react';
import { motion } from 'motion/react';
import * as React from 'react';
import type { CardOptions, CardPresetOption } from './card-settings-types';
...
function PresetCard({ preset, isActive, onClick, entityType }: PresetCardProps) {
	...
	return (
		<motion.div
			...
			whileTap={{ scale: 0.98 }}
			className={cn(
				'relative rounded-md border cursor-pointer overflow-hidden',
		...
		>
			...
			<div className="flex justify-center items-center p-2 pt-0 bg-background/50 rounded-b-md">
				<div
					style={{
			...
			<div className="absolute bottom-1 right-1 flex gap-1">
				{preset.options.enableScanlines && (
					<span className="text-purple-500 bg-purple-100 dark:bg-purple-950/30 p-0.5 rounded">
						<Layers size={10} />
					</span>
				)}
				{preset.options.enableGrainEffect && (
					<span className="text-amber-500 bg-amber-100 dark:bg-amber-950/30 p-0.5 rounded">
						<BookOpen size={10} />
					</span>
				)}
				{(preset.options.enableLightHalo || preset.options.enableAnimatedBorder) && (
					<span className="text-cyan-500 bg-cyan-100 dark:bg-cyan-950/30 p-0.5 rounded">
						<Sparkles size={10} />
					</span>
				)}
			</div>
			...
			{/* Indicador activo */}
			{isActive && (
				<div className="absolute top-0 left-0 w-full h-full border-2 border-primary rounded-md pointer-events-none" />
			)}
		</motion.div>
	);
}
