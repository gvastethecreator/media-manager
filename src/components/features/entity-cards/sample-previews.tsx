...
export function SamplePreviews({ type, onSelect, selected, className }: SamplePreviewProps) {
	...
	if (samples.length === 0) {
		return <div className="p-4 text-center text-sm text-muted-foreground">No hay muestras disponibles para {type}</div>;
	}
	...
	return (
		<div className={cn('w-full', className)}>
			<Label className="text-xs font-medium mb-2 block">
				Seleccionar{' '}
				{type === 'patterns' ? 'patrón' : type === 'filters' ? 'filtro' : type === 'textures' ? 'textura' : 'borde'}
			</Label>
			<ScrollArea className="h-[120px]">
				<div className="grid grid-cols-3 gap-2">
					{samples.map((sample) => (
						...
						>
							{sample.imageSrc && (
								<img src={sample.imageSrc} alt={sample.name} className="w-full h-full object-cover" />
							)}
							<div className="absolute inset-0 flex items-center justify-center">
								<span className="text-xs font-medium bg-background/80 px-2 py-1 rounded">{sample.name}</span>
							</div>
							{sample.id === selected && (
