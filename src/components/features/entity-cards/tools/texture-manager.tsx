...
export function TextureManager({
	...
	const handleSaveTexture = () => {
		...
		// Validar valores
		const opacity =
			currentTexture.opacity < 0
				? 0
				: currentTexture.opacity > 1
					? 1
					: currentTexture.opacity;

		const animationSpeed =
			currentTexture.animationSpeed && currentTexture.animationSpeed < 0
				? 0.1
				: currentTexture.animationSpeed && currentTexture.animationSpeed > 3
					? 3
					: currentTexture.animationSpeed;

		// Actualizar textura con el patrón seleccionado y valores validados
		const updatedTexture: TextureItem = {
			...
			blendMode: currentTexture.blendMode || "normal",
			visibleOnHover: currentTexture.visibleOnHover ?? false,
			animated: currentTexture.animated ?? false,
			layerOrder: currentTexture.layerOrder ?? 1,
		};
				...
				<DialogContent className="max-h-[90vh] overflow-y-auto">
					...
					<div className="grid gap-4">
							...
							<div className="grid grid-cols-3 gap-2">
								{svgPatterns.map((pattern) => (
									<Button
										...
										type="button"
										variant={
											currentTexture?.patternType === pattern.id
												? "default"
												: "outline"
										}
									...
									>
										<div className="w-full h-full">
											{pattern.renderSvg(currentTexture?.color || "#000000")}
										</div>
									</Button>
								))}
							</div>
						...
						<div className="space-y-2">
							<div className="flex justify-between">
								<Label htmlFor="texture-opacity">
									Opacidad: {currentTexture?.opacity?.toFixed(2) || "0.50"}
								</Label>
							</div>
							<Slider
								id="texture-opacity"
						...
						<div className="space-y-2">
							...
							<Select
								value={currentTexture?.blendMode || "normal"}
								onValueChange={(value) =>
									setCurrentTexture((prev) => {
										if (!prev) return prev;
										return {
						...
						<div className="grid gap-4 pt-2 border-t">
							...
							<div className="flex items-center justify-between">
								...
								<Switch
									id="animated"
									checked={currentTexture?.animated || false}
							...
							{currentTexture?.animated && (
								<div className="space-y-2">
									<div className="flex justify-between">
										<Label htmlFor="animation-speed">
											Velocidad de animación:{" "}
											{currentTexture?.animationSpeed?.toFixed(1) || "1.0"}
										</Label>
									</div>
									<Slider
										id="animation-speed"
										...
										max={3}
										step={0.1}
										value={[currentTexture?.animationSpeed || 1]}
										onValueChange={(values) =>
											setCurrentTexture((prev) => {
												if (!prev) return prev;
												return {
													...prev,
													animationSpeed: values[0],
												};
											})
										}
									/>
							...
							<div className="flex items-center justify-between">
								...
								<Switch
									id="visible-on-hover"
									checked={currentTexture?.visibleOnHover || false}
									onCheckedChange={(checked) =>
										setCurrentTexture((prev) => {
											if (!prev) return prev;
											return {
							...
							{currentTexture?.patternType === "noise" && (
								<>
									<div className="space-y-2">
										<Label htmlFor="noise-type">Tipo de ruido</Label>
										<Select
											value={currentTexture?.noiseType || "light"}
											onValueChange={(value) =>
												setCurrentTexture((prev) => {
													if (!prev) return prev;
													return {
									...
									<div className="space-y-2">
										<div className="flex justify-between">
											<Label htmlFor="texture-density">
												Densidad: {currentTexture?.density?.toFixed(1) || "0.6"}
											</Label>
										</div>
										<Slider
											id="texture-density"
											...
											max={1}
											step={0.1}
											value={[currentTexture?.density || 0.6]}
											onValueChange={(values) =>
												setCurrentTexture((prev) => {
													if (!prev) return prev;
													return {
									...
									<div className="space-y-2">
										<div className="flex justify-between">
											<Label htmlFor="texture-contrast">
												Contraste:{" "}
												{currentTexture?.contrast?.toFixed(1) || "1.2"}
											</Label>
										</div>
										<Slider
											id="texture-contrast"
											...
											max={2}
											step={0.1}
											value={[currentTexture?.contrast || 1.2]}
											onValueChange={(values) =>
												setCurrentTexture((prev) => {
													if (!prev) return prev;
													return {
							...
							<div className="space-y-2">
								<div className="flex justify-between">
									<Label htmlFor="layer-order">
										Orden de capa: {currentTexture?.layerOrder || 1}
									</Label>
								</div>
								<Slider
									id="layer-order"
									...
									max={10}
									step={1}
									value={[currentTexture?.layerOrder || 1]}
									onValueChange={(values) =>
										setCurrentTexture((prev) => {
											if (!prev) return prev;
											return {
								...
								<p className="text-xs text-muted-foreground">
									Define el orden de renderizado. Las capas con valores más
									altos aparecen por encima.
								</p>
							</div>

							{currentTexture?.patternType !== "none" && (
								<div className="space-y-2">
									<div className="flex justify-between">
										<Label htmlFor="texture-scale">
											Escala: {currentTexture?.scale || 1}
										</Label>
									</div>
									<Slider
										id="texture-scale"
						...
						<details className="mt-2">
							...
							<div className="space-y-2 mt-2">
								...
								/>
								<p className="text-xs text-muted-foreground">
									Edita el código SVG directamente. Ten en cuenta que esto es
						...
						<div className="space-y-2">
							<Label htmlFor="texture-description">
