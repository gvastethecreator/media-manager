...
export function AdvancedImageViewer({ images, initialIndex = 0, isOpen, onClose }: AdvancedImageViewerProps) {
  ...
  const renderThumbnails = () => {
    ...
    return images.slice(start, end).map((image, i) => {
      const gradientColors = [
        `hsl(${parseInt(image.id.split('-')[1] || '0') * 40 % 360}, 95%, 75%)`,
        `hsl(${(parseInt(image.id.split('-')[1] || '0') * 40 + 60) % 360}, 95%, 75%)`
      ]
      ...
      return (
        <motion.div
          ...
          className={cn(
            ...
            i + start === index && "ring-2 ring-primary ring-offset-2 ring-offset-background"
          )}
