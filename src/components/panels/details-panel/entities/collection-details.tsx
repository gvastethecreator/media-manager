			...
			<Card>
				...
				<CardContent className="space-y-3">
					...
					<div className="grid grid-cols-3 gap-2 pt-2 border-t">
						...
						<div className="text-center">
							<p className="text-lg font-bold text-primary">{formatBytes(entity.totalSize || 0)}</p>
						...
						{viewMode === 'grid' ? (
							<div className="grid grid-cols-2 gap-2">
								{sortedItems.map((item) => (
									...
									>
										{item.thumbnailUrl ? (
											<img src={item.thumbnailUrl} alt={item.name} className="w-12 h-12 object-cover rounded mb-1" />
										) : (
									...
									>
										...
										<div className="flex-1 min-w-0">
											...
											<p className="text-xs text-muted-foreground">
												{item.size ? formatBytes(item.size) : 'N/A'} • {item.addedAt.toLocaleDateString()}
											</p>
										</div>
										<Badge variant="outline" className="text-xs">
											{item.type}
...
export const CollectionToolbar = memo<EntityToolbarProps<CollectionWithStats>>(function CollectionToolbar({
	entity,
	onAction,
}) {
	const handleAction = useCallback(
		...
		<Card>
			...
			<CardContent>
				<div className="grid grid-cols-2 gap-2">
					...
					<Button variant="default" size="sm" onClick={() => handleAction('add-items')} className="justify-start">
						<Plus className="h-4 w-4 mr-2" />
						Añadir elementos
					</Button>

					{/* Gestión de contenido */}
					<Button variant="outline" size="sm" onClick={() => handleAction('slideshow')} className="justify-start">
						<Play className="h-4 w-4 mr-2" />
						Presentación
					</Button>
					<Button variant="outline" size="sm" onClick={() => handleAction('shuffle')} className="justify-start">
						<Shuffle className="h-4 w-4 mr-2" />
						Aleatorio
					</Button>

					{/* Organización */}
					<Button variant="outline" size="sm" onClick={() => handleAction('organize')} className="justify-start">
						<Layout className="h-4 w-4 mr-2" />
						Organizar
					</Button>
					<Button variant="outline" size="sm" onClick={() => handleAction('add-tags')} className="justify-start">
					...
					<DropdownMenu>
						...
						<DropdownMenuContent>
							...
							<DropdownMenuItem onClick={() => handleAction('duplicate')}>Duplicar colección</DropdownMenuItem>
							<DropdownMenuItem onClick={() => handleAction('export')}>Exportar</DropdownMenuItem>
