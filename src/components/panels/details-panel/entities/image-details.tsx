...
export const ImageDetails = memo<EntityDetailsProps<ImageWithStats>>(
	function ImageDetails({ entity, isSelected, onAction }) {
...
export const ImagePreview = memo<EntityPreviewProps<ImageWithStats>>(
	function ImagePreview({ entity, size = 'md', showControls = false, onAction }) {
...
export const ImageToolbar = memo<EntityToolbarProps<ImageWithStats>>(
	function ImageToolbar({ entity, onAction }) {
...
export const ImageMetadata = memo<EntityMetadataProps<ImageWithStats>>(
	function ImageMetadata({ entity, editable = false, onUpdate }) {
		if (!isImageWithStats(entity)) {
			return null;
		}
		...
		const metadata = [
			...
			},
			{
				...
				value: entity.width && entity.height
					? `${(entity.width * entity.height / 1000000).toFixed(1)}MP`
					: 'N/A',
				category: 'technical'
			},
			{
