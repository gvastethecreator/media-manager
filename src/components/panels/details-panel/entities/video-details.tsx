...
export const VideoDetails = memo<EntityDetailsProps<VideoWithStats>>(
	function VideoDetails({ entity, isSelected, onAction }) {
					...
					<CardContent className="space-y-2">
						<div className="grid grid-cols-2 gap-2 text-sm">
							...
							<div>
								...
								<p className="font-medium">
									{entity.duration ? formatTime(entity.duration) : 'N/A'}
								</p>
							</div>
							<div>
							...
							<div>
								...
								<p className="font-medium">
									{entity.frameRate ? `${entity.frameRate} fps` : 'N/A'}
								</p>
							</div>
						</div>
					</CardContent>
...
export const VideoPreview = memo<EntityPreviewProps<VideoWithStats>>(
	function VideoPreview({ entity, size = 'md', showControls = false, onAction }) {
...
export const VideoToolbar = memo<EntityToolbarProps<VideoWithStats>>(
	function VideoToolbar({ entity, onAction }) {
				...
				<CardContent>
					<div className="grid grid-cols-2 gap-2">
						...
						<Button
							variant="outline"
							size="sm"
							onClick={() => handleAction('extract-frame')}
							className="justify-start"
						>
						...
						<DropdownMenu>
							...
							<DropdownMenuContent>
								...
								<DropdownMenuItem onClick={() => handleAction('add-subtitles')}>
									Añadir subtítulos
								</DropdownMenuItem>
...
export const VideoMetadata = memo<EntityMetadataProps<VideoWithStats>>(
	function VideoMetadata({ entity, editable = false, onUpdate }) {
		if (!isVideoWithStats(entity)) {
			return null;
		}
		...
		const metadata = [
			...
			},
			{
				...
				value: entity.width && entity.height
					? (entity.width / entity.height).toFixed(2)
					: 'N/A',
				category: 'technical'
			},
			{
