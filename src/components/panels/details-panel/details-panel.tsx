...
const EntityPreview = memo<{ item: EntityWithStats }>(function EntityPreview({ item }) {
	...
	if (isImageWithStats(item) && item.thumbnailUrl) {
		return <img src={item.thumbnailUrl} alt={item.name || 'Imagen'} className="w-full h-full object-contain" />;
	}

	if (isVideoWithStats(item) && item.thumbnailUrl) {
		return (
			<div className="relative w-full h-full">
				<img src={item.thumbnailUrl} alt={item.name || 'Video'} className="w-full h-full object-contain" />
	...
	return (
		<div className="space-y-4">
			...
			<div className="text-center space-y-1">
				...
				{totalSize > 0 && <p className="text-sm text-muted-foreground">Tamaño total: {formatBytes(totalSize)}</p>}
			</div>
...
export const DetailsPanel = memo<DetailsPanelV2Props>(function DetailsPanel({ selectedItems, className }) {
		...
		<Card className={cn('h-full flex flex-col', className)}>
			<CardHeader className="flex-shrink-0 pb-3">
				<CardTitle className="flex items-center justify-between text-base">
					<span>{hasMultipleSelection ? `${selectedItems.length} elementos` : 'Detalles'}</span>
					<Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleDebug}>
						<Bug className="h-4 w-4" />
					</Button>
				</CardTitle>
			</CardHeader>

			<CardContent className="flex-1 overflow-hidden p-0">{renderContent()}</CardContent>
		</Card>
	);
});
