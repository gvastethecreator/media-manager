...
import { Calendar, Clock, FolderIcon, ImageIcon } from 'lucide-react';
...
export const FolderStats = memo(function FolderStats({ folderId, folderName }: FolderStatsProps) {
	...
	const folderStats = useMemo(() => {
		...
		const sortedByUpdated = [...folderImages].sort(
			(a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
		);

		const sortedByCreated = [...folderImages].sort(
	...
	return (
		<div className="p-4 space-y-4">
			...
			<div className="flex items-center gap-2 mb-4">
				...
				<h3 className="text-sm font-medium truncate">{folderDisplayName}</h3>
			</div>

			{/* Estadísticas principales */}
			<Card className="border-0 shadow-none bg-muted/20">
				<CardHeader className="pb-2">
					<CardTitle className="text-xs text-muted-foreground">Contenido de la carpeta</CardTitle>
				</CardHeader>
				<CardContent className="space-y-3">
					...
					<div className="flex items-center justify-between">
						...
						<span className="text-xs text-muted-foreground">{formatDate(folderStats.lastModified)}</span>
					</div>
					...
					<div className="flex items-center justify-between">
						...
						<span className="text-xs text-muted-foreground">{formatDate(folderStats.firstAdded)}</span>
					</div>
				</CardContent>
			</Card>
			...
			<Card className="border-0 shadow-none bg-muted/20">
				<CardHeader className="pb-2">
					<CardTitle className="text-xs text-muted-foreground">Información adicional</CardTitle>
				</CardHeader>
