 ...
 */
function findGenerationInfo(metadata: Record<string, any>) {
	...
	if (directFields.length >= 2) {
		...
		const generation: Record<string, any> = { type: 'unknown' };
	...
	for (const key in metadata) {
		if (typeof metadata[key] === 'object' && metadata[key] !== null) {
			...
			if (subFields.length >= 2) {
				...
				const generation: Record<string, any> = { type: key };
...
export function AIGenerationInfo({ metadata }: MetadataComponentProps) {
	...
	const [generationData, setGenerationData] = useState<Record<string, any> | null>(null);
	...
	useEffect(() => {
		if (metadata) {
			...
			if (metadata.generation) {
				...
				setGenerationData(metadata.generation);
			} else {
		...
		return (
			<div className="flex flex-col gap-2 p-2 border border-dashed border-amber-500/50 rounded-md">
				<div className="flex items-center justify-between">
					...
					<Badge
						variant="outline"
						className="text-[10px] h-5 px-2 bg-amber-500/10 text-amber-500"
					>
						No disponible
					</Badge>
				</div>
	...
	const isSD =
		generatorType.includes("stable-diffusion") || generatorType === "sd";
	const isComfyUI =
		generatorType.includes("comfyui") || generatorType === "comfy";
	const isInvokeAI =
		generatorType.includes("invoke") || generatorType === "invoke-ai";
	const isNovelAI =
		generatorType.includes("novel") || generatorType === "novel-ai";
	const isMidjourney =
		generatorType.includes("midjourney") || generatorType === "mj";
	const isDalle =
		generatorType.includes("dalle") || generatorType.includes("dall-e");
	const isUnknown =
		!generatorType ||
		(!isSD &&
			!isComfyUI &&
		...
		<div className="flex flex-col gap-2">
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<Button
						...
						className="h-5 px-2"
						onClick={() => {
							console.log("Objeto generation completo:", gen);
							toast({
								title: "Depuración",
			...
			<div className="flex flex-col gap-1.5">
				...
				<div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
					{gen.steps && (
						<InfoItem
							icon={<Code2 className="h-3.5 w-3.5 text-lime-400" />}
							label="Pasos"
							value={gen.steps}
						/>
					)}
				...
				<div className="mt-2">
					...
					<div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
						{Object.entries(gen).map(([key, value]) => {
							...
							if (['type', 'prompt', 'negative_prompt', 'model', 'steps', 'cfg_scale', 'cfg', 'seed', 'sampler', 'scheduler', 'clip_skip', 'workflow', 'extra_params'].includes(key)) {
								return null;
							}
					...
					<Button
						...
						onClick={() => {
							...
							toast({
								title: "Copiado",
								description:
									"Metadata de generación AI copiada al portapapeles",
							});
						}}
					>
