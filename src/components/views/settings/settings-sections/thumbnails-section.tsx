...
export function ThumbnailsSection() {
  ...
  const handleReprocessThumbnails = async () => {
    try {
      ...
      toast({
        ...
        description: (
          <div className="space-y-2">
            <p>{error instanceof Error ? error.message : 'Error al reprocesar las miniaturas'}</p>
  ...
  const handleOptimizeThumbnails = async () => {
    try {
      ...
      toast({
        ...
        description: (
          <div className="space-y-2">
            <p>{error instanceof Error ? error.message : 'Error al optimizar las miniaturas'}</p>
            <p className="text-sm text-muted-foreground">
              Revisa los logs para más detalles
            </p>
          </div>
  ...
  const loadStats = React.useCallback(async () => {
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Error al cargar estadísticas de miniaturas",
        variant: "destructive"
      })
    }
  }, [toast])
              ...
              {progress && (
                <div className="space-y-2">
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>
                      {progress.current} de {progress.total} ({progress.progress}%)
                    </span>
