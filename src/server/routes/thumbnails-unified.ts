 ...
 */
router.get('/image/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
 ...
 */
router.get('/video/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
 ...
 */
router.get('/audio/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
 ...
 */
router.get('/document/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
				const options = parseOptions(req.query);
				return { result: await thumbnailUnifiedService.getThumbnail('document', id, options), entityType: 'document', id };
			},
			catch: (error) => new Error(String(error)),
		}),
	{
 ...
 */
router.get('/json/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
				const options = parseOptions(req.query);
				return { result: await thumbnailUnifiedService.getThumbnail('jsonFile', id, options), entityType: 'jsonFile', id };
			},
			catch: (error) => new Error(String(error)),
		}),
	{
 ...
 */
router.get('/3d/:id', effectHandler(
	(req) =>
		Effect.tryPromise({
			try: async () => {
				const { id } = req.params;
				const options = parseOptions(req.query);
				return { result: await thumbnailUnifiedService.getThumbnail('file3d', id, options), entityType: 'file3d', id };
			},
			catch: (error) => new Error(String(error)),
		}),
	{
 ...
 */
router.post('/generate', effectHandler((req) =>
	Effect.tryPromise({
		try: async () => {
 ...
 */
router.post('/batch', effectHandler((req) =>
	Effect.tryPromise({
		try: async () => {
 ...
 */
router.get('/info/:entityType/:entityId', effectHandler((req) =>
	Effect.tryPromise({
		try: async () => {
