...
import { loadThumbnailStatRows } from '@/services/thumbnail/thumbnail-stats.service';
import {
	...
	effectHandler((req) =>
		Effect.tryPromise({
			try: async () => {
				...
				const { audioRows, documentRows, file3dRows, imageRows, jsonRows, videoRows } =
					await loadThumbnailStatRows();
