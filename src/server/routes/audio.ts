...
const AudioFiltersSchema = z.object({
  ...
  sortBy: z.enum(['name', 'createdAt', 'updatedAt', 'size', 'duration', 'bitrate', 'artist', 'album']).default('name').optional(),
...
const audioInclude = {
  ...
  _count: {
    select: {
      albums: true, tags: true, characters: true, collections: true,
      places: true, worldItems: true, concepts: true, prompts: true, notes: true,
      wildcards: true, properties: true, groups: true,
    }
  }
};
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({
      data: audioFiles,
      pagination: {
        total, limit: filters.limit, offset: filters.offset,
    ...
    res.status(500).json({ error: 'Error interno del servidor', message: error instanceof Error ? error.message : 'Error desconocido' });
  }
});
