...
export class WildcardTransformer {
	...
	static async findMany(options: {
		...
		try {
			...
			let extendedItems = items.map((item) => {
				const extendedItem = WildcardTransformer.extend(item as WildcardBase);
				if (item.children) {
	 ...
	 */
	static async findById(id: string, include?: Record<string, boolean>): Promise<WildcardWithRelations | null> {
		try {
			...
			const extendedWildcard = WildcardTransformer.extend(wildcard as WildcardBase);
	 ...
	 */
	static async findByCategory(category: string, limit = 10): Promise<WildcardWithRelations[]> {
		try {
			...
			return wildcards.map((wildcard) => WildcardTransformer.extend(wildcard as WildcardBase));
		} catch (error) {
	 ...
	 */
	static async findFavorites(limit = 10): Promise<WildcardWithRelations[]> {
		try {
			const wildcards = await prisma.wildcard.findMany({
	 ...
	 */
	static async findChildren(parentId: string, limit = 100): Promise<WildcardWithRelations[]> {
		try {
			const wildcards = await prisma.wildcard.findMany({
				where: { parentId },
				take: limit,
	...
	static async create(data: CreateWildcardData): Promise<WildcardWithRelations> {
		try {
			...
			// Extender comodín con campos deserializados
			return WildcardTransformer.extend(wildcard as WildcardBase);
		} catch (error) {
	...
	static async update(id: string, data: UpdateWildcardData): Promise<WildcardWithRelations> {
		try {
			...
			// Extender comodín con campos deserializados
			return WildcardTransformer.extend(updatedWildcard as WildcardBase);
		} catch (error) {
	...
	static async delete(id: string): Promise<WildcardWithRelations> {
		try {
			...
			// Extender comodín con campos deserializados
			return WildcardTransformer.extend(deletedWildcard as WildcardBase);
		} catch (error) {
