...
export class PropertyTransformer {
	...
	static async findMany(options: {
		...
		try {
			...
			const extendedItems = items.map((item) => PropertyTransformer.extend(item as PropertyBase));
	 ...
	 */
	static async findById(id: string, include?: Record<string, boolean>): Promise<PropertyWithRelations | null> {
		try {
			...
			return PropertyTransformer.extend(property as PropertyBase);
		} catch (error) {
	...
	static async update(id: string, data: UpdatePropertyData): Promise<PropertyWithRelations> {
		try {
			...
			// Extender propiedad con campos deserealizados
			return PropertyTransformer.extend(updatedProperty as PropertyBase);
		} catch (error) {
	...
	static async delete(id: string): Promise<PropertyWithRelations> {
		try {
			...
			// Extender propiedad con campos deserealizados
			return PropertyTransformer.extend(deletedProperty as PropertyBase);
		} catch (error) {
	 ...
	 */
	static async findByCategory(category: string, limit = 10): Promise<PropertyWithRelations[]> {
		try {
			...
			return properties.map((property) => PropertyTransformer.extend(property as PropertyBase));
		} catch (error) {
	 ...
	 */
	static async findFavorites(limit = 10): Promise<PropertyWithRelations[]> {
		try {
			const properties = await prisma.property.findMany({
