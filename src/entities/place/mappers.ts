...
export function mapCreatePlaceDataToPrisma(data: CreatePlaceData): Prisma.PlaceCreateInput {
  try {
    const createData: Prisma.PlaceCreateInput = {
      ...
      dangers: typeof data.dangers === 'string' ?
        data.dangers :
        serializePlaceDangers(data.dangers || null),
      resources: typeof data.resources === 'string' ?
        data.resources :
        serializePlaceResources(data.resources || null),
      lore: data.lore,
      history: data.history,
      stats: typeof data.stats === 'string' ?
        data.stats :
        serializePlaceStats(data.stats || null),
      sortBy: data.sortBy,
      filters: typeof data.filters === 'string' ?
        data.filters :
        serializePlaceFilters(data.filters || null),
...
export function mapUpdatePlaceDataToPrisma(data: Partial<PlaceComplete>): Prisma.PlaceUpdateInput {
  try {
    ...
    if (data.dangers !== undefined) {
      updateData.dangers = typeof data.dangers === 'string' ?
        data.dangers :
        serializePlaceDangers(data.dangers);
    }
    if (data.resources !== undefined) {
      updateData.resources = typeof data.resources === 'string' ?
        data.resources :
        serializePlaceResources(data.resources);
    }
