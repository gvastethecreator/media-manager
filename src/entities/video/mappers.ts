...
export function mapUpdateVideoDataToPrisma(data: Partial<VideoComplete>): Prisma.VideoUpdateInput {
	try {
		...
		if (data.metadata !== undefined) {
			updateData.metadata = typeof data.metadata === 'string' ? data.metadata : serializeVideoMetadata(data.metadata);
		}
		...
		if (data.playState !== undefined) {
			updateData.playState =
				typeof data.playState === 'string' ? data.playState : serializeVideoPlayState(data.playState);
		}
		if (data.chapters !== undefined) {
			updateData.chapters = typeof data.chapters === 'string' ? data.chapters : serializeVideoChapters(data.chapters);
		}
