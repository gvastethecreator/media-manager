...
export async function getFile3DById(id: string): Promise<File3D | null> {
  // TODO: Obtener de DB
  return null;
}

export async function updateFile3D(id: string, input: unknown): Promise<File3D> {
  const file = validateFile3D(input);
  // TODO: Actualizar en DB
  return file;
}

export async function deleteFile3D(id: string): Promise<boolean> {
  // TODO: Eliminar de DB
  return true;
}
