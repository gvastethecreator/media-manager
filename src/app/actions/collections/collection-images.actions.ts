...
export async function addImageToCollection(imageId: string, collectionId: string) {
	try {
		...
		throw new Error(
			`No se pudo añadir la imagen a la colección: ${error instanceof Error ? error.message : String(error)}`
		);
	}
}
...
export async function removeImageFromCollection(imageId: string, collectionId: string) {
	try {
		...
		throw new Error(
			`No se pudo eliminar la imagen de la colección: ${error instanceof Error ? error.message : String(error)}`
		);
	}
}
