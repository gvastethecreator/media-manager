...
interface ExifTag {
	id: number;
	value: string | number | number[] | boolean | Date | null;
	description?: string;
}
...
const _ITXT_CHUNK = 'iTXt';
const _ZTXT_CHUNK = 'zTXt';
...
// Crear una interfaz para representar el resultado de parseValue
...
class MetadataService {
	...
	private calculateDelay(baseDelay: number, attempt: number, config: RetryConfig): number {
		const exponentialDelay = Math.min(baseDelay * config.backoffFactor ** (attempt - 1), config.maxDelay);
	...
	private async extractPNGMetadata(path: string): Promise<PNGMetadata> {
		try {
	...
	private async extractExifMetadata(tags: ExifReader.Tags): Promise<Partial<FileMetadata>> {
		...
		if (tags.Make || tags.Model || tags.Software) {
			...
			if (tags.GPSLatitude && tags.GPSLongitude) {
				const gpsData = {
					latitude: this.parseGPSCoordinate(tags.GPSLatitude as unknown as TypedGPSTag, tags.GPSLatitudeRef as ExifTag),
			...
			for (const [key, pattern] of Object.entries(patterns)) {
				...
				if (match) {
					...
					if (key in result && value !== undefined) {
						const typedResult = result as Record<string, ParsedValue>;
	...
	private parseValue(key: string, value: string): ParsedValue {
	...
	private validateMetadata(metadata: FileMetadata): boolean {
		try {
			...
			if (metadata.generation) {
				const generationData = metadata.generation;

				// Validar tipo
				if (
					!generationData.type ||
					!['stable-diffusion', 'comfyui', 'invoke-ai', 'novel-ai'].includes(generationData.type)
				) {
					this.logger.warn('Metadata inválida: tipo de generación AI desconocido', { type: generationData.type });
					return false;
				}

				// Validar campos requeridos según el tipo
				if (generationData.type === 'stable-diffusion' && !generationData.prompt) {
					this.logger.warn('Metadata inválida: falta prompt en metadata de Stable Diffusion');
					return false;
				}

				// Validar tipos de datos
				if (generationData.steps && !Number.isInteger(generationData.steps)) {
					this.logger.warn('Metadata inválida: steps debe ser un número entero');
					return false;
				}

				if (generationData.cfg_scale && typeof generationData.cfg_scale !== 'number') {
					this.logger.warn('Metadata inválida: cfg_scale debe ser un número');
					return false;
				}

				if (generationData.seed && !Number.isInteger(generationData.seed)) {
...
export async function preloadMetadata(paths: string[]): Promise<void> {
	try {
		...
		const results = await Promise.allSettled(paths.map((path) => metadataService.extractMetadata(path)));
