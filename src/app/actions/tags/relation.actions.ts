	...
	try {
		...
		for (const path of REVALIDATE_PATHS) {
			revalidatePath(path);
                ...
                await serverEmit({ type: 'stats:change' as EventType, data: { entity: 'tags' } }); // Usar serverEmit
...
export async function removeTagFromImages(tagId: string, imageIds: string[]): Promise<void> {
	try {
		...
		throw createRelationError(
			'No se pudo eliminar el tag de las imágenes',
			TagRelationErrorCode.OPERATION_FAILED,
			error
		);
	}
}
...
export async function getSuggestedTags(imageId: string, limit = 10): Promise<string[]> {
	try {
		...
		throw createRelationError(
			'No se pudieron obtener sugerencias de tags',
			TagRelationErrorCode.OPERATION_FAILED,
			error
		);
	}
...
export async function updateImageTags(imageId: string, tagIds: string[]): Promise<void> {
	try {
		...
		throw createRelationError(
			'No se pudieron actualizar los tags de la imagen',
			TagRelationErrorCode.OPERATION_FAILED,
			error
		);
	}
