...
export async function indexFolder(id: string): Promise<FolderResponse> {
	try {
		...
		const { processed, total, totalSize } = await processDirectory(folder.path, folder.id, (status) => {
			emitProgress(status);
		});
	...
	try {
		...
		const processDirectory = async (
			...
			try {
				...
				const existingFiles = new Map(folder.images.map((img) => [img.path, img]));
...
// Definición de las interfaces relacionadas
	...
	try {
		...
		return {
			...
			collections: Array.isArray(image.collections)
				? image.collections.map((c: ImageEntity) => ({
						id: c?.id || '',
						name: c?.name || '',
					}))
				: [],
			tags: Array.isArray(image.tags)
				? image.tags.map((t: ImageEntity) => ({
						id: t?.id || '',
						name: t?.name || '',
					}))
				: [],
			albums: Array.isArray(image.albums)
				? image.albums.map((a: ImageEntity) => ({
						id: a?.id || '',
						name: a?.name || '',
					}))
				: [],
			characters: Array.isArray(image.characters)
				? image.characters.map((c: ImageEntity) => ({
						id: c?.id || '',
						name: c?.name || '',
					}))
				: [],
			places: Array.isArray(image.places)
				? image.places.map((p: ImageEntity) => ({
						id: p?.id || '',
						name: p?.name || '',
					}))
				: [],
			objects: Array.isArray(image.objects)
				? image.objects.map((o: ImageEntity) => ({
						id: o?.id || '',
						name: o?.name || '',
					}))
				: [],
