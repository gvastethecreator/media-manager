...
export async function checkFolderTableStructure(): Promise<{
  ...
  try {
    ...
    diagnosticsLogger.info('✅ Estructura de la tabla Folder verificada correctamente',
...
export async function checkFolderConsistency(id: string): Promise<FolderDiagnosticResult> {
  try {
    ...
    if (error &&
        typeof error === 'object' &&
        'name' in error &&
        error.name === 'FolderDiagnosticError') {
      throw error;
    }
...
export async function analyzeFolderHealth(id: string): Promise<FolderHealthResult> {
  try {
    ...
    const missingFilesIssue = diagnostic.issues.find(i => i.message.includes('imágenes en la base de datos no existen'));
    const missingFiles = missingFilesIssue
      ? Number.parseInt(missingFilesIssue.message.split(' ')[0])
      : 0;
    ...
    const inconsistentStats = diagnostic.issues.some(i => i.type === 'contents' && i.message.includes('Inconsistencia'));
...
export async function getDuplicateFiles(options?: {
  ...
  try {
    ...
    const fileDetails = fileIds.length > 0
