...
export async function GET_active() {
  try {
    ...
    return NextResponse.json(
      { error: 'Error obteniendo perfil activo' },
      { status: 500 }
    );
  }
}
...
export async function PUT_active(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: 'ID del perfil es requerido' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Error actualizando perfil activo' },
      { status: 500 }
    );
  }
}
...
export async function DELETE_id(
  ...
  try {
    ...
    if (profileCount <= 1) {
      return NextResponse.json(
        { error: 'No se puede eliminar el último perfil' },
        { status: 400 }
      );
    }
...
export async function PATCH_preferences(
  request: Request,
  { params }: { params: { id: string } }
) {
  const id = params.id;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Error actualizando preferencias del perfil' },
      { status: 500 }
    );
  }
}