...
export async function GET() {
	try {
		...
		return NextResponse.json({ error: 'Error al obtener objetos del mundo' }, { status: 500 });
	}
}
...
export async function POST(request: Request) {
	try {
		...
		// Validación básica
		if (!data.name) {
			return NextResponse.json({ error: 'El nombre del objeto es obligatorio' }, { status: 400 });
		}
		...
		return NextResponse.json({ error: 'Error al crear objeto del mundo' }, { status: 500 });
	}
}
