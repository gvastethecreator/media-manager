...
export async function GET(request: NextRequest) {
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: 'Error al obtener conceptos' }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!data.name) {
      ...
      return new NextResponse(
        JSON.stringify({ error: 'Se requiere al menos un nombre para el concepto' }),
    ...
    return new NextResponse(
      JSON.stringify({ error: 'Error al crear concepto' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}