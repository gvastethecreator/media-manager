...
export async function GET(request: NextRequest) {
	try {
		...
		return new NextResponse(JSON.stringify({ error: 'Error al obtener etiquetas' }), {
			status: 500,
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!data.name) {
			...
			return new NextResponse(JSON.stringify({ error: 'Se requiere al menos un nombre para la etiqueta' }), {
				status: 400,
				headers: { 'Content-Type': 'application/json' },
			});
		}
		...
		return new NextResponse(JSON.stringify({ error: 'Error al crear etiqueta' }), {
			status: 500,
			headers: { 'Content-Type': 'application/json' },
		});
	}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
	try {
		...
		return new NextResponse(JSON.stringify({ error: 'Error al actualizar etiqueta' }), {
			status: 500,
			headers: { 'Content-Type': 'application/json' },
		});
	}
...
export async function DELETE(request: NextRequest) {
	try {
		...
		return new NextResponse(JSON.stringify({ error: 'Error al eliminar etiqueta' }), {
			status: 500,
			headers: { 'Content-Type': 'application/json' },
		});
	}
