...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }): Promise<NextResponse> {
	try {
		...
		if (!settings) {
			return NextResponse.json({ error: `No se encontró perfil con ID: ${profileId}` }, { status: 404 });
		}
		...
		return NextResponse.json({ error: 'Error al obtener la configuración del perfil' }, { status: 500 });
	}
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }): Promise<NextResponse> {
	try {
		...
		return NextResponse.json({ error: 'Error al actualizar la configuración del perfil' }, { status: 500 });
	}
}
