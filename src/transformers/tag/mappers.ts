...
export function toSearchOptions(options: TagSearchOptions): Prisma.TagFindManyArgs {
	try {
		const { skip = 0, take = DEFAULT_PAGE_SIZE, orderBy, where: filters = {}, include = {} } = options;
		...
		const orderByMapped = orderBy
			? Object.entries(orderBy).map(([field, direction]) => ({ [field]: direction }))
			: [{ createdAt: 'desc' }];
		...
		// Mapear inclusiones
		const includeRelations: Prisma.TagInclude = {
			_count: true,
		};

		// Incluir solo las relaciones solicitadas
		if (include.images) includeRelations.images = { select: { id: true } };
		if (include.videos) includeRelations.videos = { select: { id: true } };
		if (include.albums) includeRelations.albums = { select: { id: true } };
		if (include.collections) includeRelations.collections = { select: { id: true } };
		if (include.characters) includeRelations.characters = { select: { id: true } };
		if (include.places) includeRelations.places = { select: { id: true } };
		if (include.worldItems) includeRelations.worldItems = { select: { id: true } };
		if (include.concepts) includeRelations.concepts = { select: { id: true } };
		if (include.prompts) includeRelations.prompts = { select: { id: true } };
		if (include.notes) includeRelations.notes = { select: { id: true } };
		if (include.wildcards) includeRelations.wildcards = { select: { id: true } };
		if (include.properties) includeRelations.properties = { select: { id: true } };
		if (include.groups) includeRelations.groups = { select: { id: true } };
...
export function toSearchResult(
	...
	try {
		...
		const items = tags.map((tag) => fromPrismaTag(tag as any));
