...
export function toCreateTagData(data: TagCreateInput): Prisma.TagCreateInput {
  try {
    ...
    throw new TransformerError(
      'TagTransformer',
      'Error mapeando datos de creación',
      { cause: error }
    );
  }
}
...
export function toUpdateTagData(data: TagUpdateInput): Prisma.TagUpdateInput {
  try {
    ...
    throw new TransformerError(
      'TagTransformer',
      'Error mapeando datos de actualización',
      { cause: error }
    );
  }
}
...
export function toSearchResult(
  ...
  try {
    ...
    const items = tags.map(tag => fromPrismaTag(tag, {
      includeUI: true,
      includeStats: true,
    ...
    throw new TransformerError(
      'TagTransformer',
      'Error generando resultado de búsqueda',
      { cause: error }
    );
  }
}
