...
function calculateQualityScore(thumbnail: ThumbnailBase): number {
	...
	if (thumbnail.width >= 300 && thumbnail.height >= 300) score += 20;
	else if (thumbnail.width >= 150 && thumbnail.height >= 150) score += 10;

	// Bonus por formato
	if (thumbnail.format === 'webp') score += 15;
	else if (thumbnail.format === 'png') score += 10;
	else if (thumbnail.format === 'jpg' || thumbnail.format === 'jpeg') score += 5;
...
function calculateStorageEfficiency(thumbnail: ThumbnailBase): number {
	if (thumbnail.size <= 0 || thumbnail.width <= 0 || thumbnail.height <= 0) return 0;
	...
	if (bytesPerPixel < 1) return 100;
	if (bytesPerPixel < 2) return 80;
	if (bytesPerPixel < 3) return 60;
	if (bytesPerPixel < 4) return 40;
	return 20;
}
