...
export function transformThumbnail<T extends Record<string, any>>(
    ...
    // Fechas
    const createdAt = thumbnail.createdAt instanceof Date
        ? thumbnail.createdAt
        : new Date(thumbnail.createdAt || thumbnail.created_at || Date.now());

    const updatedAt = thumbnail.updatedAt instanceof Date
        ? thumbnail.updatedAt
    ...
    const errorTimestamp = thumbnail.errorTimestamp instanceof Date
        ? thumbnail.errorTimestamp
        : thumbnail.errorTimestamp
            ? new Date(thumbnail.errorTimestamp)
            : null;

    const optimizedAt = thumbnail.optimizedAt instanceof Date
    ...
    if (includeMetadata && thumbnail.metadata) {
        try {
            metadata = typeof thumbnail.metadata === 'string'
                ? JSON.parse(thumbnail.metadata)
                : thumbnail.metadata;
        } catch (error) {
 ...
 */
export function transformThumbnailToWithStats(thumbnail: ThumbnailComplete, allThumbnails: ThumbnailComplete[]): ThumbnailWithStats {
...
export function transformThumbnailToExtended(thumbnail: ThumbnailComplete): ThumbnailExtended {
    ...
    const optimizationStatus = hasFailed
        ? 'error'
        : thumbnail.optimizedAt
            ? 'optimized'
            : 'not-optimized';
