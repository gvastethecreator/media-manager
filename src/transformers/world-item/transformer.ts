...
export function transformWorldItemToExtended(worldItem: WorldItem): WorldItemExtended {
  try {
    ...
    return {
      ...
      attributesArray: typeof baseItem.attributes === 'string'
        ? JSON.parse(baseItem.attributes || '[]')
        : baseItem.attributes || [],
      effectsArray: typeof baseItem.effects === 'string'
        ? JSON.parse(baseItem.effects || '[]')
        : baseItem.effects || [],
      requirementsArray: typeof baseItem.requirements === 'string'
        ? JSON.parse(baseItem.requirements || '[]')
        : baseItem.requirements || [],
      statsObject: typeof baseItem.stats === 'string'
        ? JSON.parse(baseItem.stats || '{}')
        : baseItem.stats || {}
    };
  } catch (error) {
...
export function transformWorldItemToWithStats(worldItem: WorldItem): WorldItemWithStats {
  try {
    ...
    logger.error('Error transformando WorldItem a versión con estadísticas:', { error, worldItemId: (worldItem as any)?.id });
...
function generateStatsDisplay(worldItem: WorldItem): Array<{name: string, value: number}> {
  try {
    ...
    const stats = typeof worldItem.stats === 'string'
      ? JSON.parse(worldItem.stats || '{}')
      : worldItem.stats || {};
