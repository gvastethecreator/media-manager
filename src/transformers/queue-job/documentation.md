# Documentación de Transformadores de QueueJob

## Descripción

Los transformadores de **QueueJob** permiten mapear, serializar, deserializar y extender la entidad QueueJob para distintos usos (UI, API, persistencia, estadísticas, etc.), asegurando siempre el uso de tipos canónicos y validación robusta.

---

## Diagrama de Flujo (Mermaid)

```mermaid
flowchart TD
    A[QueueJob (Drizzle/Raw)] --> B[queue-job-transformers.ts]
    B -->|toQueueJobListItem| C[QueueJobListItem]
    B -->|toQueueJobCard| D[QueueJobCard]
    B -->|parseQueueJobSearchParams| E[Drizzle.QueueJobWhereInput]
```

---

## Estructura y Relaciones

- **queue-job-transformers.ts**: Mapeo, serialización y validación.
- **index.ts**: Barrel limpio, solo exporta funciones y tipos canónicos.

---

## Ejemplo de Uso

```typescript
import { toQueueJobListItem } from '@/transformers/queue-job/queue-job-transformers';

const listItem = toQueueJobListItem(rawQueueJob);
```

---

## Buenas Prácticas

- Usar **solo** los tipos y funciones canónicas exportadas.
- No modificar los tipos base ni duplicar lógica de transformación.
- Validar siempre los datos con los esquemas y funciones provistas.
- Mantener el barrel (`index.ts`) limpio y sin duplicados.

---

## Notas

- Todos los mapeos gestionan errores y validaciones de forma robusta.
- No existen tipos legacy ni duplicados en este módulo.

---

## Última revisión

- Fecha: 2025-06-10
- Estado: ✅ Auditado, sin errores TS, documentación y diagramas actualizados.
