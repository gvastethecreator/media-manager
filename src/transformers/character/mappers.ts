...
export function toCharacterListItem(
    ...
    const extended = 'isSelected' in character
        ? character as CharacterExtended
        : toCharacterExtended(character as PrismaCharacter);
 ...
 */
export function toCharacterCard(
    character: PrismaCharacter | CharacterExtended
): CharacterCard {
    const extended = 'isSelected' in character
        ? character as CharacterExtended
...
export function mapPrimaryStats(character: CharacterExtended): CharacterAttribute[] {
    ...
    if (mappedStats.length < 6) {
        ...
        for (const key in stats) {
            ...
            const isPrimary = primaryStats.some(
                stat => key.toLowerCase() === stat
            );
    ...
    if (mappedStats.length < defaultStats.length) {
        return [
            ...mappedStats,
            ...defaultStats.slice(mappedStats.length)
        ];
    }
...
export function mapCharacterInventory(character: CharacterExtended): CharacterInventoryItem[] {
    ...
    const classInventory = characterClass && inventoryByClass[characterClass]
        ? inventoryByClass[characterClass]
        : [];
 ...
 */
export function getSuggestedAppearance(
 ...
 */
export function charactersToListItems(
 ...
 */
export function charactersToCards(
    characters: (PrismaCharacter | CharacterExtended)[]
): CharacterCard[] {
