			...
			include:
				Object.keys(include).length > 0
					? {
							...include,
							_count: {
								select: Object.keys(include).reduce(
									(acc, key) => {
										acc[key] = true;
										return acc;
									},
...
export function mapNoteSearchOptionsToPrisma(options: NoteSearchOptions): Prisma.NoteFindManyArgs {
	try {
		const { page = 1, pageSize = 25, sortBy = 'updatedAt', sortOrder = 'desc', includeRelations = false } = options;
 ...
 */
export function mapNoteToRelatedNote(note: NoteComplete, count = 1, strength = 1): RelatedNote {
	try {
		return {
			id: note.id,
