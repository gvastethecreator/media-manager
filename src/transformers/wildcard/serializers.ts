...
import type { WildcardBase, WildcardDeserialized, WildcardUI } from '@/types/entities/wildcard/types';
...
export function fromPrismaWildcard<T extends WildcardBase>(
	...
	try {
		...
		const { includeRelations = false, includeUI = false, includeStats = false, deserializeFields = true } = options;
		...
		if (includeUI) {
			result._ui = {
				...
				lastUpdated: wildcard.updatedAt instanceof Date ? wildcard.updatedAt : new Date(wildcard.updatedAt),
			};
		}
