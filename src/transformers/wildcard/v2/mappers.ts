...
export function toCreateWildcardData(data: CreateWildcardData): any {
  try {
    ...
    throw new TransformerError(
      'WildcardTransformer',
      'Error mapeando datos de creación de wildcard',
      { cause: error }
    );
  }
}
...
export function toUpdateWildcardData(data: UpdateWildcardData): any {
  try {
    ...
    throw new TransformerError(
      'WildcardTransformer',
      'Error mapeando datos de actualización de wildcard',
      { cause: error }
    );
  }
}
...
export function toBulkUpdateWildcardData(data: WildcardBulkUpdateData): any {
  try {
    ...
    throw new TransformerError(
      'WildcardTransformer',
      'Error mapeando datos de actualización en lote de wildcards',
      { cause: error }
    );
  }
}
...
export function toWildcardRelated(wildcard: WildcardBase): WildcardRelated {
  return {
    ...
    isFavorite: 'favorite' in wildcard
      ? (wildcard as any).favorite
      : (wildcard as any).isFavorite || false
  };
}
...
export function toSearchOptions(options: WildcardSearchOptions = {}): any {
  try {
    ...
    throw new TransformerError(
      'WildcardTransformer',
      'Error mapeando opciones de búsqueda',
      { cause: error }
    );
  }
}
...
function applyFilters(where: Record<string, any>, filters: WildcardSearchFilters): void {
  ...
  if (filters.hasImages !== undefined) {
    where.images = filters.hasImages
      ? { some: {} }
      : { none: {} };
  }

  if (filters.hasVideos !== undefined) {
    where.videos = filters.hasVideos
      ? { some: {} }
      : { none: {} };
  }

  if (filters.hasAlbums !== undefined) {
    where.albums = filters.hasAlbums
      ? { some: {} }
      : { none: {} };
  }

  if (filters.hasTags !== undefined) {
    where.tags = filters.hasTags
      ? { some: {} }
      : { none: {} };
  }
