    ...
    if (data.name) {
      ...
      if (existingAlbum) {
        throw createAlbumError(
          `Ya existe un álbum con el nombre "${data.name}"`,
          AlbumErrorCode.ALREADY_EXISTS
        );
      }
    }
...
export const updateAlbumService = async (id: string, data: AlbumUpdateInput): Promise<AlbumComplete> => {
  try {
    ...
    if (!existingAlbum) {
      throw createAlbumError(
        `No se encontró el álbum con ID: ${id}`,
        AlbumErrorCode.NOT_FOUND
      );
    }
...
export const getAlbumStatsService = async (id: string): Promise<AlbumWithStats> => {
  try {
    ...
    const totalItems = albumData._count.images + albumData._count.videos +
                     albumData._count.characters + albumData._count.places +
                     albumData._count.worldItems + albumData._count.concepts;
...
export const addImageToAlbumService = async (albumId: string, imageId: string): Promise<void> => {
  try {
    ...
    if (!album) {
      throw createAlbumError(
        `No se encontró el álbum con ID: ${albumId}`,
        AlbumErrorCode.NOT_FOUND
      );
    }
...
export const removeImageFromAlbumService = async (albumId: string, imageId: string): Promise<void> => {
  try {
    ...
    throw createAlbumError(`Error al eliminar imagen del álbum: ${error.message}`, AlbumErrorCode.OPERATION_FAILED, error);
  }
};