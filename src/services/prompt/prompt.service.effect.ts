...
export interface PromptServiceInterface {
	...
	readonly removeImage: (id: string, imageId: string) => Effect.Effect<void, PromptError>;
...
const make = (): PromptServiceInterface => {
				...
				try: () =>
					db
						...
						.values({
							...
							content: restInput.content ?? null,
				...
				try: () =>
					db
						...
						.set({
							...
							...(restInput.content !== undefined && { content: restInput.content }),
							...
							...(restInput.type !== undefined && { type: restInput.type }),
							updatedAt: new Date(),
						})
				...
				try: () =>
					db.transaction(async (transaction: FavoriteWriteTransaction) => {
						const deleteRows = async () => {
							...
							const deleted = await transaction.delete(prompts).where(eq(prompts.id, id)).returning();
						...
						return allowFileBacked
							? withTaxonomyArtifactMutationPermit(transaction, 'prompt', id, 'delete', deleteRows)
							: deleteRows();
					}),
	...
	const delete_ = (id: string): Effect.Effect<void, PromptError> => deleteEntity(id, false);
	...
	const addImage = (id: string, imageId: string): Effect.Effect<void, PromptError> =>
		Effect.gen(function* () {
			yield* getById(id);
			yield* Effect.tryPromise({
				try: () => db.insert(imagePrompts).values({ A: imageId, B: id }).onConflictDoNothing(),
				catch: (error) => fromUnknownPromptError('addImage', error),
			});
		});

	const removeImage = (id: string, imageId: string): Effect.Effect<void, PromptError> =>
		Effect.gen(function* () {
			yield* getById(id);
			yield* Effect.tryPromise({
				try: () => db.delete(imagePrompts).where(and(eq(imagePrompts.A, imageId), eq(imagePrompts.B, id))),
				catch: (error) => fromUnknownPromptError('removeImage', error),
			});
		});
