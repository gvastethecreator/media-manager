 ...
 * @description Extrae metadata de todos los archivos indexados
 */

import { serverLogger } from '@/lib/logger/server-logger';
import type { ReindexAnalysisResult, ReindexOptions, ReindexPhaseResult } from '../folder-reindex-types';

const logger = serverLogger.withContext('ReindexPhase7');
...
export async function phase7_extractMetadata(
	...
	try {
		const { extractMetadata } = await import('@/services/metadata/metadata.service');
		...
		for (const image of imagesWithoutMetadata) {
			try {
				...
				await extractMetadata(image.path);
