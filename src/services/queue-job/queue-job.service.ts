...
export async function cancelQueueJob(id: string): Promise<QueueJobExtended> {
  try {
    ...
    if (
      existingJob.status !== QueueJobStatus.PENDING &&
      existingJob.status !== QueueJobStatus.RETRYING
    ) {
 ...
 */
export async function findQueueJobsByStatus(
