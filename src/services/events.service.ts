...
export class EventService {
  ...
  private readonly DEFAULT_HEARTBEAT_TIMEOUT = 30000; // 30 segundos
  private readonly DEFAULT_RECONNECT_INTERVAL = 1000; // 1 segundo
    ...
    try {
      this.eventSource = new EventSource(endpoint, {
        ...
        headers: {
          ...
          'Client-Id': `client-${Date.now()}` // Identificador único para el cliente
        },
  ...
  private handleError(error: any, endpoint: string, config: EventSourceConfig): void {
    ...
    if (this.shouldReconnect()) {
      ...
      this.logger.info(`🔄 Reintentando conexión (${this.reconnectAttempts}/${this.MAX_RECONNECT_ATTEMPTS})...`);
      setTimeout(() => {
        this.connect(endpoint, config);
  ...
  on(eventType: string, callback: EventCallback): void {
    ...
    this.logger.debug(`📝 Registrado nuevo handler para evento: ${eventType}`);
  }
  ...
  off(eventType: string, callback: EventCallback): void {
    if (this.eventHandlers[eventType]) {
      ...
      this.logger.debug(`🗑️ Eliminado handler para evento: ${eventType}`);
    }
  }
