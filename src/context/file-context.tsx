...
export function FilesProvider({ children }: { children: React.ReactNode }) {
	...
	const handleSelectCollection = async (id: string) => {
		...
		try {
			...
			console.log(
				`✨ Encontrados ${collectionFiles.length} archivos en la colección ${collection.name}`
			);
	...
	const handleSelectFolder = async (id: string) => {
		...
		try {
			...
			console.log(
				`📑 Encontrados ${folderFiles.length} archivos en la carpeta ${folder.name}`
			);
	...
	const handleSelectTag = async (name: string) => {
		...
		try {
			...
			const tagFiles = mockFiles.filter((file) => file.tags.includes(tag.name));
			console.log(
				`🔍 Encontrados ${tagFiles.length} archivos con la etiqueta ${tag.name}`
			);
	...
	const handleSelectItem = (item: FileItem) => {
		try {
			console.log("🖼️ Seleccionando archivo:", {
				id: item.id,
				name: item.name,
			});
