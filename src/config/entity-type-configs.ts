 ...
 */
const MEDIA_OPERATIONS: ContextMenuAction[] = [
  ...BASE_OPERATIONS,
  'preview',
  'download',
  'open-in-explorer',
 ...
 */
const CONTAINER_OPERATIONS: ContextMenuAction[] = [
  'open',
  'rename',
  'delete',
  'move',
 ...
 */
const RELATIONAL_OPERATIONS: ContextMenuAction[] = [
  'open',
  'delete',
  'copy',
  'favorite-toggle',
...
export function isFormatSupported(type: EntityStatsType, format: string): boolean {
  ...
  return config.supportedFormats.some(supportedFormat =>
    normalizedFormat.endsWith(supportedFormat.toLowerCase())
  );
}
...
export function detectEntityTypeFromExtension(filename: string): EntityStatsType | null {
  ...
  for (const [type, config] of Object.entries(ENTITY_TYPE_CONFIGS)) {
    if (config.supportedFormats?.some(format =>
      format.toLowerCase() === extensionWithDot
    )) {
