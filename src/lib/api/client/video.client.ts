...
export interface FindVideosOptions { filters?: Partial<VideoFilters>; folderIds?: string[]; }

export async function findVideosInApi(options: FindVideosOptions = {}): Promise<VideoWithStats[]> {
    const params = new URLSearchParams();
    if (options.folderIds) params.append('folderIds', options.folderIds.join(','));
    ...
    if (!response.ok) throw new Error('Error al buscar videos');
    return response.json();
}
