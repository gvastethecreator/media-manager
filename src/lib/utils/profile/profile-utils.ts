...
export function validateProfilePreferences(preferences: Record<string, unknown>): Partial<ProfilePreferences> {
  ...
  // Valores booleanos
  const booleanFields = [
    'enableAnimations', 'enableSounds', 'enableHaptics', 'enableNotifications',
    'showHiddenFiles', 'highContrast', 'reducedMotion', 'outlineElements'
  ];
  ...
  if (preferences.defaultView &&
      ['grid', 'list', 'gallery', 'compact'].includes(preferences.defaultView as string)) {
    validatedPreferences.defaultView = preferences.defaultView as "grid" | "list" | "gallery" | "compact";
  }

  // Ordenación por defecto
  if (preferences.defaultSort &&
      ['name', 'date', 'size', 'type'].includes(preferences.defaultSort as string)) {
  ...
  if (preferences.fontSize &&
      ['small', 'medium', 'large'].includes(preferences.fontSize as string)) {
