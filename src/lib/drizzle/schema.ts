...
export const queueJobs = sqliteTable(
  ...
  (table) => ({
    queueStatusIdx: index('QueueJob_queue_status_idx').on(
      table.queue,
      table.status,
    ),
    statusCreatedIdx: index('QueueJob_status_createdAt_idx').on(
      table.status,
      table.createdAt,
    ),
    priorityStatusCreatedIdx: index(
      'QueueJob_priority_status_createdAt_idx',
    ).on(table.priority, table.status, table.createdAt),
...
export const folders = sqliteTable(
  'Folder',
  {
    ...
    autoReindex: integer('autoReindex', { mode: 'boolean' })
      .notNull()
      .default(false),
...
export const images = sqliteTable(
  ...
  (table) => ({
    pathFolderIdIdx: uniqueIndex('Image_path_folderId_key').on(
      table.path,
      table.folderId,
    ),
...
export const metadatas = sqliteTable(
  ...
  (table) => ({
    entityTypeEntityIdIdx: index('Metadata_entityType_entityId_idx').on(
      table.entityType,
      table.entityId,
    ),
...
export const thumbnails = sqliteTable(
  'Thumbnail',
  {
    ...
    isGenerated: integer('isGenerated', { mode: 'boolean' })
      .notNull()
      .default(true),
    createdAt: integer('createdAt', { mode: 'timestamp_ms' })
      .notNull()
  ...
  (table) => ({
    entityTypeEntityIdSizeIdx: uniqueIndex(
      'Thumbnail_entityType_entityId_size_key',
    ).on(table.entityType, table.entityId, table.size),
