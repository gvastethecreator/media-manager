 ...
 */
export async function getQueueJobs(filters: QueueJobFilters = {}, pagination: QueueJobPaginationOptions = {}) {
	try {
...
export async function createQueueJob(data: CreateQueueJobInput) {
	try {
		...
		const jobData = typeof data.data === 'string' ? data.data : stringifyQueueJobData(data.data);

		const jobMetadata = data.metadata ? stringifyQueueJobMetadata(data.metadata) : null;
...
export async function updateQueueJob(id: string, data: UpdateQueueJobInput) {
	try {
		...
		if (data.status !== undefined) {
			...
			if (
				(data.status === QueueJobStatus.COMPLETED || data.status === QueueJobStatus.FAILED) &&
		...
		if (data.data !== undefined) {
			updateData.data = typeof data.data === 'string' ? data.data : stringifyQueueJobData(data.data);
		}
		...
		if (data.metadata !== undefined) {
			updateData.metadata = data.metadata ? stringifyQueueJobMetadata(data.metadata) : null;
		}
