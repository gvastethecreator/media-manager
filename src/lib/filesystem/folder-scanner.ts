 ...
 */
export async function scanFolder(folderPath: string, options: ScanFolderOptions = {}): Promise<FolderScanResult> {
	// Normalizar ruta
	...
	try {
		...
		// Escanear la carpeta
		await scanDirectory(normalizedPath, normalizedPath, result, {
			recursive,
			currentDepth: 0,
			maxDepth,
			includeExtensions,
 ...
 */
function sortFileList(files: FileInfo[], sortBy: 'name' | 'size' | 'date', direction: 'asc' | 'desc'): void {
...
async function processFileEntry(
	...
	try {
		...
		// Protección contra rutas extremadamente largas
		if (fullPath.length > 1000) {
			// Si es extremadamente largo (como el base64 del error), lo saltamos
			scannerLogger.warn('Saltando archivo con ruta extremadamente larga:', {
