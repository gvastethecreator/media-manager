...
export class EnterExitCoordinator {
	private readonly activeTransitions = new Map<string, ActiveTransition>();
	private readonly groups = new Map<string, TransitionGroupConfig>();
	 ...
	 */
	async coordinateEnter(elements: ElementEntry[], globalConfig: Partial<EnterConfig> = {}): Promise<void> {
	 ...
	 */
	async coordinateExit(elements: ElementEntry[], globalConfig: Partial<ExitConfig> = {}): Promise<void> {
		if (this.isProcessing) {
			return new Promise((resolve) => {
				this.queue.push(async () => {
	...
	private calculateStaggerDelays(
		...
		switch (groupConfig.staggerType) {
			case 'equal':
				elements.forEach((el, index) => {
					const order = groupConfig.staggerDirection === 'reverse' ? elements.length - 1 - index : index;
	 ...
	 */
	private coordinateDirections(elements: ElementEntry[], type: 'enter' | 'exit'): Map<string, TransitionDirection> {
