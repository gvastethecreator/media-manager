 ...
 */
function calculateRectDiff(first: DOMRect, last: DOMRect): { x: number; y: number; scaleX: number; scaleY: number } {
...
function parseBorderRadius(radius: string): { x: number; y: number } {
	const values = radius.split(/\s+/).map((v) => Number.parseFloat(v) || 0);
	return {
		x: values[0] || 0,
...
export class FlipEngine {
	 ...
	 */
	private async animateElement(id: string): Promise<void> {
		const config = this.elements.get(id);
		const state = this.states.get(id);
		...
		// Animar border-radius si está habilitado (en paralelo)
