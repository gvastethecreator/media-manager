...
function getViewportRelativePosition(point: Point2D): {
	...
	const horizontal = point.x < viewportWidth / 3 ? 'left' : point.x > (viewportWidth * 2) / 3 ? 'right' : 'center';

	const vertical = point.y < viewportHeight / 3 ? 'top' : point.y > (viewportHeight * 2) / 3 ? 'bottom' : 'center';
...
export class DirectionTracker {
	 ...
	 */
	calculateEntryDirection(targetRect: DOMRect, fromPosition?: Point2D, previousRect?: DOMRect): TransitionDirection {
	 ...
	 */
	calculateDisplacementVector(direction: TransitionDirection, distance: number): { x: number; y: number } {
		const vectors: Record<TransitionDirection, Point2D> = {
			top: { x: 0, y: -distance },
			bottom: { x: 0, y: distance },
			left: { x: -distance, y: 0 },
			right: { x: distance, y: 0 },
			'top-left': { x: -distance * Math.SQRT1_2, y: -distance * Math.SQRT1_2 },
			'top-right': { x: distance * Math.SQRT1_2, y: -distance * Math.SQRT1_2 },
			'bottom-left': { x: -distance * Math.SQRT1_2, y: distance * Math.SQRT1_2 },
			'bottom-right': { x: distance * Math.SQRT1_2, y: distance * Math.SQRT1_2 },
			center: { x: 0, y: 0 },
			auto: { x: 0, y: distance },
		};
 ...
 */
export function predictPosition(currentPosition: Point2D, velocity: Point2D, timeDelta: number): Point2D {
	return {
