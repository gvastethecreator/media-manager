...
import { undoRedoManager, type UndoRedoState, type UndoableAction } from '@/services/undo-redo/undo-redo-manager';
...
export function useUndoRedo(options: UseUndoRedoOptions = {}): UseUndoRedoReturn {
  ...
  // Execute an undoable action
  const execute = useCallback(async (action: UndoableAction) => {
