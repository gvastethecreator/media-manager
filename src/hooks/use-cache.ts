...
import { getCacheManager, CacheManager, CacheConfig, CacheStatistics, CacheEvents } from '../services/cache/cache-manager';
...
export const useCache = (options: UseCacheOptions = {}): CacheHookResult => {
  ...
  const set = useCallback(<T>(key: string, data: T, ttl?: number, tags?: string[]): boolean => {
    try {
  ...
  // Batch operations
  const getMultiple = useCallback(<T>(keys: string[]): Map<string, T> => {
    try {
  ...
  const setMultiple = useCallback(<T>(entries: Map<string, T>, ttl?: number, tags?: string[]): boolean => {
    try {
  ...
  // Tag operations
  const getByTag = useCallback((tag: string): Map<string, any> => {
    try {
  ...
  // File-specific operations
  const cacheFileItem = useCallback((item: FileItem, ttl?: number): boolean => {
    try {
  ...
  const getCachedFileItem = useCallback((id: string): FileItem | null => {
    try {
  ...
  const cacheDirectoryListing = useCallback((path: string, items: FileItem[], ttl?: number): boolean => {
    try {
  ...
  const getCachedDirectoryListing = useCallback((path: string): FileItem[] | null => {
    try {
  ...
  const cacheFileStats = useCallback((id: string, stats: EntityStatsType, ttl?: number): boolean => {
    try {
  ...
  const getCachedFileStats = useCallback((id: string): EntityStatsType | null => {
    try {
  ...
  const cacheThumbnail = useCallback((id: string, thumbnail: string, ttl?: number): boolean => {
    try {
  ...
  const cacheSearchResults = useCallback((query: string, results: FileItem[], ttl?: number): boolean => {
    try {
  ...
  const getCachedSearchResults = useCallback((query: string): FileItem[] | null => {
    try {
  ...
  const evictLRU = useCallback((count?: number): number => {
    try {
  ...
  const evictLFU = useCallback((count?: number): number => {
    try {
