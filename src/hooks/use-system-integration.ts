...
export const useSystemIntegration = (
  options: SystemIntegrationOptions = {}
): UseSystemIntegrationReturn => {
  ...
  const checkCapabilities = useCallback(async () => {
    ...
    setIsSupported(
      newCapabilities.hasFileSystemAccess ||
      newCapabilities.hasClipboardAccess ||
      newCapabilities.hasNotificationAccess
    );
   ...
   */
  const selectInExplorer = useCallback(async (path: string): Promise<boolean> => {
   ...
   */
  const copyToClipboard = useCallback(async (items: AnyEntityWithStats[]): Promise<boolean> => {
   ...
   */
  const unregisterFileHandler = useCallback((extension: string): boolean => {
    try {
