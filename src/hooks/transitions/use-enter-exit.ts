...
export function useEnterExit(options: UseEnterExitOptions): UseEnterExitReturn {
  ...
  useEffect(() => {
    if (isControlled && controlledVisible !== isVisible) {
   ...
   */
  const performEnter = useCallback(async (direction?: TransitionDirection): Promise<void> => {
   ...
   */
  const performExit = useCallback(async (direction?: TransitionDirection): Promise<void> => {
    if (!enabled || !ref.current) {
      setIsVisible(false);
      return;
    }
...
interface UseEnterExitGroupReturn {
  registerElement: (id: string, index: number, config?: { enter?: EnterConfig; exit?: ExitConfig }) => {
    ref: React.RefObject<HTMLElement | null>;
    id: string;
  };
...
export function useEnterExitGroup(options: UseEnterExitGroupOptions): UseEnterExitGroupReturn {
   ...
   */
  const enterAll = useCallback(async (direction?: TransitionDirection): Promise<void> => {
    if (!enabled) return;
   ...
   */
  const exitAll = useCallback(async (direction?: TransitionDirection): Promise<void> => {
    if (!enabled) return;
