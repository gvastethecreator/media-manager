...
export function useMorph(options: UseMorphOptions): UseMorphReturn {
	...
	const morphTo = useCallback(
		async (shape: string, overrideConfig?: Partial<MorphConfig>): Promise<void> => {
	...
	const morph = useCallback(
		async (fromShape: string, toShape: string, overrideConfig?: Partial<MorphConfig>): Promise<void> => {
...
export function useMorphLoop(options: {
	...
	const start = useCallback(() => {
		...
		stopRef.current = engine.current.createContinuousMorph(ref.current, shapes, { ...config, interval });
		setIsRunning(true);
