...
import { getFlipEngine, getEnterExitCoordinator } from '@/lib/transitions';
import type { TransitionDirection, EnterConfig, ExitConfig } from '@/lib/transitions';
 ...
 * ```
 */
export function useEntityCardTransition(
  options: UseEntityCardTransitionOptions
): UseEntityCardTransitionReturn {
   ...
   */
  const handleCardClick = useCallback(async (callback: () => void): Promise<void> => {
   ...
   */
  const handleSelectionChange = useCallback(async (selected: boolean): Promise<void> => {
   ...
   */
  const handleExpandChange = useCallback(async (expanded: boolean): Promise<void> => {
    if (!enabled || !cardRef.current) return;
 ...
 */
export function useEntityCardGroupTransition(options: {
  groupId: string;
  enabled?: boolean;
  staggerDelay?: number;
}): {
