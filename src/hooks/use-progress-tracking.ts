...
export function useProgressTracking(
  ...
  const startOperation = useCallback(
    (type: OperationType, progressOptions: Partial<ProgressOperation>): string => {
  ...
  const updateProgress = useCallback(
    (operationId: string, progress: ProgressInfo, stepId?: string) => {
  ...
  const getOperation = useCallback(
    (operationId: string): ProgressOperation | undefined => {
...
export function useBatchOperation() {
  ...
  const updateBatchProgress = useCallback(
    (operationId: string, processedCount: number, currentItem?: string) => {
      const operation = progressService.getOperation(operationId);
      if (operation) {
  ...
  const completeBatchOperation = useCallback(
    (operationId: string) => {
      progressService.completeOperation(operationId);
      setBatchOperations(prev => {
        const newMap = new Map(prev);
  ...
  const failBatchOperation = useCallback(
    (operationId: string, error: string) => {
      progressService.failOperation(operationId, error);
      setBatchOperations(prev => {
        const newMap = new Map(prev);
