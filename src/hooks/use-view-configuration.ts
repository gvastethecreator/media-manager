...
import { ListViewSettings } from '@/types/file-browser/view-configuration';
...
export function useViewConfiguration(viewType: ViewType): UseViewConfigurationReturn {
  ...
  const updateConfiguration = useCallback((updates: Partial<ViewConfiguration>) => {
  ...
  const updateCommonSettings = useCallback((commonUpdates: Partial<CommonViewSettings>) => {
    updateConfiguration({
      common: {
  ...
  const updateSpecificSettings = useCallback((specificUpdates: Partial<ViewSpecificSettings['config']>) => {
    updateConfiguration({
      specific: {
  ...
  const updateViewConfig = useCallback((updates: Partial<ViewConfiguration>) => {
  ...
  const updateEntityConfig = useCallback((updates: any) => {
    updateConfiguration({
      ...currentConfiguration,
      ...updates,
    });
  ...
  const applyPresetAsync = useCallback(async (presetId: string): Promise<boolean> => {
    try {
      const preset = availablePresets.find(p => p.id === presetId);
      if (preset) {
        updateConfiguration(preset.configuration);
  ...
  const importConfigurationAsync = useCallback(async (configJson: string, options?: any): Promise<{ success: boolean; errors: string[] }> => {
    try {
