...
const transformToFileItem = (rawItem: any): FileItem => {
  try {
    ...
    const fileItem: FileItem = {
      ...
      metadata: typeof rawItem.metadata === 'string' ? rawItem.metadata :
                rawItem.metadata ? JSON.stringify(rawItem.metadata) : null,
      ...
      collections: Array.isArray(rawItem.collections)
        ? rawItem.collections.map((c: any) => typeof c === 'string' ? { id: c, name: c } : { id: c.id, name: c.name || c.id })
        : [],
      tags: Array.isArray(rawItem.tags)
        ? rawItem.tags.map((t: any) => typeof t === 'string'
          ? { id: t, name: t, color: '#94a3b8' }
          : { id: t.id, name: t.name || t.id, color: t.color || '#94a3b8' })
        : [],
      albums: Array.isArray(rawItem.albums)
        ? rawItem.albums.map((a: any) => typeof a === 'string' ? { id: a, name: a } : { id: a.id, name: a.name || a.id })
        : [],
      characters: Array.isArray(rawItem.characters)
        ? rawItem.characters.map((c: any) => typeof c === 'string' ? { id: c, name: c } : { id: c.id, name: c.name || c.id })
        : [],
      places: Array.isArray(rawItem.places)
        ? rawItem.places.map((p: any) => typeof p === 'string' ? { id: p, name: p } : { id: p.id, name: p.name || p.id })
        : [],
      worldItems: Array.isArray(rawItem.worldItems)
        ? rawItem.worldItems.map((w: any) => typeof w === 'string' ? { id: w, name: w } : { id: w.id, name: w.name || w.id })
        : [],
...
export const useUnifiedFileManager = create<UnifiedFileManagerState>((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      let stats = folderCache.get(cacheKey);      if (!stats) {
  ...
  selectRange: (fromIndex: number, toIndex: number) => {
    ...
    set({
      selectedItems: [...state.selectedItems, ...rangeItems.filter(item =>
  ...
  refreshCurrentContext: async () => {
    ...
    const contextId = state.currentFolderId || state.currentCollectionId ||
                     state.currentTagId || state.currentAlbumId ||
                     state.currentCharacterId || state.currentPlaceId ||
