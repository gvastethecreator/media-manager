...
export type BaseStore<T extends BaseEntity, CreateType = Partial<T>, UpdateType = Partial<T>> =
  BaseState<T> & BaseActions<T, CreateType, UpdateType>;

export type ExtendedStore<T extends BaseEntity, S = {}, CreateType = Partial<T>, UpdateType = Partial<T>> =
  BaseStore<T, CreateType, UpdateType> & S;

export type StoreCreator<T extends BaseEntity, S = {}, CreateType = Partial<T>, UpdateType = Partial<T>> =
  StateCreator<ExtendedStore<T, S, CreateType, UpdateType>, [], [], ExtendedStore<T, S, CreateType, UpdateType>>;

export type StoreHook<T extends BaseEntity, S = {}, CreateType = Partial<T>, UpdateType = Partial<T>> = {
