    ...
    const baseStore: BaseStore<T, CreateType, UpdateType> = {
      ...
      createItem: async (data: CreateType) => {
        try {
          ...
          // Hook antes de crear
          const processedData = options.actions?.beforeCreate
            ? await options.actions.beforeCreate(data)
            : data;
      ...
      updateItem: async (id: string, data: UpdateType) => {
        try {
          ...
          const processedData = options.actions?.beforeUpdate
            ? await options.actions.beforeUpdate(id, data)
            : data;
