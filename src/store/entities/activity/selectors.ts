...
import type { Activity, ActivityCategory } from '@/types/entities/activity';
...
export const selectIsActivitySelected = (id: string) => (state: ActivityState) =>
...
export const selectIsActivityExpanded = (id: string) => (state: ActivityState) =>
  state.ui.expandedIds.includes(id);
...
export const selectFilteredActivities = (state: ActivityState) => {
  ...
  if (filters.onlyAlerts) {
    activities = activities.filter(activity =>
      activity.type.startsWith('system_error') ||
      activity.type.startsWith('system_warning'));
  }

  // Filtrar por imagen
  if (filters.filterByImageId) {
    activities = activities.filter(activity =>
      activity.imageId === filters.filterByImageId);
  }
