...
import { type ActivityCategory, type ActivityFilters, ActivitySortCriteria } from '../../../../types/entities/activity';
...
> = (set, get) => ({
	...
	removeCategoryFilter: (category: ActivityCategory) => {
		set((state) => ({
			filters: {
				...state.filters,
				selectedCategories: state.filters.selectedCategories.filter((cat) => cat !== category),
			},
		}));
	},
