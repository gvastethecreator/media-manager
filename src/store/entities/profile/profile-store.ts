...
export const selectTheme = (state: ProfileStore) =>
  state.activeProfile?.theme || ThemeMode.SYSTEM;

// Selector para obtener preferencias específicas
export const selectPreference = <K extends keyof ProfilePreferences>(key: K) =>
  (state: ProfileStore) => state.activeProfile?.preferences?.[key];

// Selector para obtener el color del perfil activo
export const selectProfileColor = (state: ProfileStore) =>
  state.activeProfile?.color || '#3b82f6';

// Selector para verificar si hay un perfil activo
export const selectHasActiveProfile = (state: ProfileStore) =>
  !!state.activeProfile;
