...
export const selectFavoriteCharacters = (): CharacterExtended[] =>
    useCharacterStore(state =>
        Object.values(state.characters).filter(character => character.isFavorite)
    );
 ...
 */
export const selectViewConfig = (): CharacterViewConfig =>
    useCharacterStore(state => state.viewConfig);

/**
 * Obtiene los IDs de personajes que están expandidos
 * @returns Array de IDs de personajes expandidos
 */
export const selectExpandedCharacterIds = (): string[] =>
    useCharacterStore(state => state.expandedCharacterIds);

/**
 * Obtiene la opción de ordenamiento actual
 * @returns Opción de ordenamiento
 */
export const selectCurrentSortOption = (): CharacterSortOption =>
    useCharacterStore(state => state.currentSortOption);