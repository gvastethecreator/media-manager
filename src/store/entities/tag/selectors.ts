	...
	switch (sortBy) {
		...
		case TagSortCriteria.USAGE_ASC:
			return tagsCopy.sort((a, b) => {
				const aCount = a._count ? (a._count.images || 0) + (a._count.videos || 0) : 0;
				const bCount = b._count ? (b._count.images || 0) + (b._count.videos || 0) : 0;
...
export const useFilteredTags = () => {
	...
	return filterByCategory(filterBySearchTerm(items, searchTerm), category);
};
		...
		mostUsedTag:
			tags.length > 0
				? tags.reduce((prev, current) => {
						const prevUsage = prev._count ? (prev._count.images || 0) + (prev._count.videos || 0) : 0;
						const currentUsage = current._count ? (current._count.images || 0) + (current._count.videos || 0) : 0;
