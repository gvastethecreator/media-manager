...
const validateDataTypes = (item: any): void => {
  // Validar tipos básicos
  if (typeof item.id !== 'string') throw new Error('id debe ser string')
  if (typeof item.name !== 'string') throw new Error('name debe ser string')
  if (typeof item.path !== 'string') throw new Error('path debe ser string')
  if (typeof item.size !== 'number') throw new Error('size debe ser number')
  if (typeof item.folderId !== 'string') throw new Error('folderId debe ser string')

  // Validar tipos opcionales
  if (item.type && typeof item.type !== 'string') throw new Error('type debe ser string')
  if (item.width && typeof item.width !== 'number') throw new Error('width debe ser number')
  if (item.height && typeof item.height !== 'number') throw new Error('height debe ser number')
  if (item.isPublic && typeof item.isPublic !== 'boolean') throw new Error('isPublic debe ser boolean')
  if (item.isFavorite && typeof item.isFavorite !== 'boolean') throw new Error('isFavorite debe ser boolean')
  ...
  // Validar arrays
  const validateArray = (arr: any[] | undefined, name: string) => {
...
const validateRequiredFields = (item: any): void => {
...
const transformToFileItem = (rawItem: any): FileItem => {
  try {
    ...
    const fileItem: FileItem = {
      ...
      metadata: typeof item.metadata === 'string' ? item.metadata : item.metadata ? JSON.stringify(item.metadata) : null,
