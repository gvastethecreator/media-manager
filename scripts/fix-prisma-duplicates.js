...
function cleanDuplicateAudioFields() {
  ...
  const duplicatePattern = /(\s+\/\/ Metadata\s+createdAt DateTime @default\(now\(\)\)\s+updatedAt DateTime @updatedAt\s+)Audio\s+Audio\?\s+@relation\(fields: \[audioId\], references: \[id\]\)\s+audioId\s+String\?\s+Audio\s+Audio\?\s+@relation\(fields: \[audioId\], references: \[id\]\)\s+/g;
