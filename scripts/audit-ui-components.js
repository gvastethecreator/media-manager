...
function analyzeComponent(filePath, fileName) {
  ...
  const hasBaseUIPatterns = content.includes('data-[checked]') || content.includes('data-[starting-style]') || content.includes('--available-height');
...
function main() {
  ...
  results.filter(r => r.hasBaseUIImport).forEach(r => {
    console.log(`  • ${r.file} - ${r.details}`);
  });
  console.log();

  console.log('⏳ COMPONENTES RADIX (PENDIENTES):');
  results.filter(r => r.hasRadixImport).forEach(r => {
    console.log(`  • ${r.file} - ${r.details}`);
  });
  console.log();

  console.log('🔧 COMPONENTES CUSTOM:');
  results.filter(r => !r.hasRadixImport && !r.hasBaseUIImport).forEach(r => {
    console.log(`  • ${r.file} - ${r.details}`);
  });
  console.log();
