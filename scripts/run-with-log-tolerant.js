...
const child = spawn(
  ...
  isWindows ? ['/c', fullCommand] : ['-c', fullCommand],
  {
...
child.on('close', (code) => {
  ...
  if (code === 0) {
    ...
    console.log(
      chalk.yellow.bold(`⚠️  ${tipo} completado con issues encontrados (Exit code: ${code})`)
    );
    console.log(
      chalk.cyan(
        `🔍 Esto es normal para herramientas de ${tipo.toLowerCase()} cuando encuentran problemas`
      )
    );
    console.log(chalk.gray(`📄 Log completo guardado en: ${logFilePath}`));
    ...
    if (missingDep) {
      console.log(
        chalk.yellow('🛈 Parece que faltan dependencias. Ejecuta "pnpm install" e intenta de nuevo.')
      );
    }
    process.exit(code);
  }
