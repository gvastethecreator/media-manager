...
// filepath: d:\DEV\image-manager\scripts\migrations\clean-world-item-json-fields-fixed.ts
...
function cleanJsonField(field: string | null, fieldName: string, defaultValue = '[]'): { value: string; wasFixed: boolean } {
	...
	try {
		...
		serverLogger.debug(`🔄 Intentando reparar campo ${fieldName}: "${field.substring(0, 50)}${field.length > 50 ? '...' : ''}"`);
		...
		if (['attributes', 'effects', 'requirements', 'stats'].includes(fieldName)) {
			...
			if (repairedAttribute) {
				try {
					...
					serverLogger.info(`✅ Campo ${fieldName} reparado como atributos: "${field}" → ${repairedAttribute}`);
					return { value: repairedAttribute, wasFixed: true };
				} catch (error) {
					serverLogger.error(`❌ Error al parsear atributos reparados: ${repairedAttribute}`, error);
				}
			}
		}
		...
		if (field.length > 0 && !field.startsWith('[') && !field.startsWith('{')) {
			try {
				...
				serverLogger.info(`✅ Campo ${fieldName} envuelto en array: "${field}" → ${wrappedAsArray}`);
				return { value: wrappedAsArray, wasFixed: true };
			} catch (error) {
				serverLogger.debug(`❌ No se pudo envolver en array: ${field}`, error);
			}
		}

		// 🚨 Si todas las estrategias fallan, usar valor por defecto
		serverLogger.warn(`❌ No se pudo reparar campo ${fieldName}. Campo: "${field}", Error original: ${originalError}. Usando valor por defecto: ${defaultValue}.`);
		return { value: defaultValue, wasFixed: true };
	}
}
...
async function cleanWorldItemJsonFields(): Promise<CleanupStats> {
	...
	try {
		serverLogger.info('🧹 Iniciando limpieza de campos JSON en WorldItem...');
		...
		serverLogger.info(`📊 Total de registros a procesar: ${stats.totalRecords}`);
		...
		for (const item of worldItems) {
			try {
				...
				if (itemFixed) {
					...
					stats.fixedRecords++;
					serverLogger.debug(`✅ WorldItem "${item.name}" (${item.id}) actualizado`);
				}

			} catch (error) {
				stats.errors++;
				serverLogger.error(`❌ Error procesando WorldItem "${item.name}" (${item.id}):`, error);
			}
		}

		serverLogger.info('🎉 Limpieza de campos JSON completada exitosamente!');
		serverLogger.info('📊 Estadísticas de limpieza:', {
		...
		serverLogger.error('❌ Error durante la migración:', error);
		stats.errors++;
		throw error;
	}
}
...
async function verifyCleanup(): Promise<void> {
	serverLogger.info('🔍 Verificando resultados de la limpieza...');
	...
	if (Array.isArray(problematicItems) && problematicItems.length > 0) {
		serverLogger.warn(`⚠️  Aún hay ${problematicItems.length} registros con posibles problemas:`, problematicItems);
	} else {
		serverLogger.info('✅ Verificación completada: No se encontraron más problemas JSON');
	}
}
...
if (require.main === module) {
	cleanWorldItemJsonFields()
		...
		.catch(async (error) => {
			serverLogger.error('💥 Error fatal en la migración:', error);
			await prisma.$disconnect();
			process.exit(1);
		});
}
