...
// Fondos
const backgrounds = ['gradient-background', 'grid-background', 'hover-background'];
...
async function tryInstallSingleSlug(slug: string): Promise<{ ok: boolean; message?: string }> {
	const url = `https://reui.io/r/${slug}.json`;
...
async function attemptPrimary(slug: string): Promise<'installed' | 'not-found' | 'failed' | 'skipped'> {
...
async function attemptFallbacks(slug: string): Promise<'installed' | 'not-found' | 'failed'> {
