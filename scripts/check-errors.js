...
async function filterRelevantFiles(files, options) {
  ...
  if (relevantFiles.length === 0) {
    console.log(
      chalk.green(`✅ No se encontraron logs para '${options.tool}' en el período especificado.`)
    );
    return null;
  }
...
async function checkErrors() {
  ...
  console.log(
    chalk.cyan(
      `🔍 Analizando logs de ${chalk.bold(options.tool)} de los últimos ${chalk.bold(options.days)} día(s)...`
    )
  );
