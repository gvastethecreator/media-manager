...
async function main() {
	...
	try {
		...
		const propertyVariations = tablesResult.filter((t: any) => 
			t.name.toLowerCase().includes('property') || t.name.toLowerCase().includes('properties')
		);
		const wildcardVariations = tablesResult.filter((t: any) => 
			t.name.toLowerCase().includes('wildcard')
		);
