...
async function main() {
	...
	try {
		...
		const expectedPropertyColumns = [
			'id', 'name', 'description', 'emoji', 'color', 'category',
			'isPublic', 'isFavorite', 'totalImages', 'totalVideos',
			'createdAt', 'updatedAt'
		];
		
		const actualPropertyColumns = tableInfo.map((col: any) => col.name);
		const missingPropertyColumns = expectedPropertyColumns.filter(
			col => !actualPropertyColumns.includes(col)
		);
		...
		const missingWildcardColumns = expectedWildcardColumns.filter(
			col => !actualWildcardColumns.includes(col)
		);
