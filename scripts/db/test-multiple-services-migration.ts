...
async function testMultipleServicesMigration() {
	...
	try {
		...
		const time = Date.now() - start;
		console.log(
			`   ✅ getConcepts: ${time}ms - Total: ${conceptsResult.total}, Concepts: ${conceptsResult.items.length}`
		);
	} catch (error) {
	...
	console.log(
		`   Tag getById: ${tagResult.status === 'fulfilled' ? '✅' : '❌'} - ${tagResult.status === 'fulfilled' ? (tagResult.value ? 'Found' : 'null (esperado)') : 'Error'}`
	);
	console.log(
		`   Album getById: ${albumResult.status === 'fulfilled' ? '✅' : '❌'} - ${albumResult.status === 'fulfilled' ? (albumResult.value ? 'Found' : 'null (esperado)') : 'Error'}`
	);
	console.log(
		`   Concept getById: ${conceptResult.status === 'fulfilled' ? '✅' : '❌'} - ${conceptResult.status === 'fulfilled' ? (conceptResult.value ? 'Found' : 'null (esperado)') : 'Error'}`
	);
