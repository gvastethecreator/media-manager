...
async function testNoteService() {
	...
	try {
		...
		// Notas favoritas
		const favoriteNotes = await db.select({ count: count() }).from(notes).where(eq(notes.isFavorite, true));
		console.log(`   ⭐ Notas favoritas: ${favoriteNotes[0]?.count || 0}`);

		// Notas por categoría (si hay)
		const categorizedNotes = await db.select({ count: count() }).from(notes).where(eq(notes.category, 'general'));
		console.log(`   📂 Notas categoría 'general': ${categorizedNotes[0]?.count || 0}`);

		// Notas por prioridad alta
		const highPriorityNotes = await db.select({ count: count() }).from(notes).where(eq(notes.priority, 3));
		console.log(`   🔥 Notas prioridad alta (3): ${highPriorityNotes[0]?.count || 0}`);

		// Notas por estado activo
		const activeNotes = await db.select({ count: count() }).from(notes).where(eq(notes.status, 'active'));
