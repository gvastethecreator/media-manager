...
async function debugJoinIssue() {
  ...
  try {
    ...
    const activeProfileRaw = await db.select()
      .from(profiles)
    ...
    if (activeProfile.settingsId) {
      ...
      const settingsById = await db.select()
        .from(settings)
        .where(eq(settings.id, activeProfile.settingsId));
    ...
    const inverseJoin = await db.select({
      // Profile fields
      profileId: profiles.id,
      profileName: profiles.name,
      profileSettingsId: profiles.settingsId,
