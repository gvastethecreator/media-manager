...
const fixes = [
  {
    ...
    search: 'const transformedTags = includeStats\n\t\t\t? tags.map(toTagWithStats)\n\t\t\t: tags.map(tag => ({\n\t\t\t\t...tag,\n\t\t\t\tstats: {\n\t\t\t\t\ttotalRelations: 0,\n\t\t\t\t\tusageDiversity: 0,\n\t\t\t\t\tpopularity: 0,\n\t\t\t\t\tcompletenessScore: 0,\n\t\t\t\t}\n\t\t\t}));',
    replace: 'const transformedTags = includeStats\n\t\t\t? tags.map(toTagWithStats)\n\t\t\t: tags.map(tag => ({\n\t\t\t\t...tag,\n\t\t\t\tstats: {\n\t\t\t\t\ttotalRelations: 0,\n\t\t\t\t\tusageDiversity: 0,\n\t\t\t\t\tpopularity: 0,\n\t\t\t\t\tcompletenessScore: 0,\n\t\t\t\t}\n\t\t\t} as TagWithStats));'
  },
  {
    file: 'src/types/thumbnails.ts',
    search: 'export interface LastProcessedThumbnail {\n\tid: string;\n\tpath: string;\n\tprocessedAt: Date;\n\tstatus: \'success\' | \'error\';\n\terror?: string;\n}',
    replace: 'export interface LastProcessedThumbnail {\n\tid: string;\n\tpath: string;\n\tprocessedAt: Date;\n\tstatus: \'success\' | \'error\';\n\terror?: string;\n}'
  }
];
