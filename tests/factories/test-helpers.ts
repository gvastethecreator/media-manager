 ...
 */
export async function expectSuccess<E, A>(effect: Effect.Effect<A, E, never>): Promise<A> {
	const either = await Effect.runPromise(Effect.either(effect));
	if (either._tag === 'Right') {
		return either.right;
	}
	throw new Error(`Expected success but got failure: ${JSON.stringify(either.left)}`);
}
 ...
 */
export async function expectError<E, A>(effect: Effect.Effect<A, E, never>, expectedTag?: string): Promise<E> {
	const either = await Effect.runPromise(Effect.either(effect));
	if (either._tag === 'Left') {
		if (expectedTag !== undefined) {
			const error = either.left as any;
			if (error._tag !== expectedTag) {
				throw new Error(`Expected error tag "${expectedTag}" but got "${error._tag}"`);
			}
		}
		return either.left;
	}
