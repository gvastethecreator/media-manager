		...
		if (await tableExists(prisma, 'Character')) {
			const characters = [
				...
				},
				{
					...
					description: 'Explorador de dimensiones prohibidas, marcado por la oscuridad.',
					emoji: '🕳️',
					color: '#0ea5e9',
					category: 'mystic',
					level: 40,
					...
					backstory: 'Desaparecido en una expedición, regresó con poderes y secretos prohibidos.',
					stats: JSON.stringify({ strength: 14, dexterity: 18, intelligence: 17, constitution: 12, wisdom: 15, charisma: 10 }),
					abilities: JSON.stringify(['Shadow Step', 'Abyssal Gaze']),
					psychologicalProfile: 'Enigmático, reservado, obsesionado con el conocimiento prohibido.',
					socialProfile: 'Pocos amigos, muchos secretos.',
					relationships: JSON.stringify([]),
					goals: JSON.stringify(['Descubrir el origen del Abismo']),
					fears: JSON.stringify(['Ser consumido por la oscuridad']),
					beliefs: JSON.stringify(['El conocimiento es poder']),
					personality: JSON.stringify(['Enigmático', 'Obsesivo']),
					skills: JSON.stringify(['Exploración', 'Magia prohibida']),
					sortBy: 'level',
