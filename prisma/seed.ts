 ...
 */
async function safeDeleteMany(model: string, tableName: string): Promise<void> {
  try {
...
async function main() {
  ...
  for (const { model, table } of modelsToClean) {
    await safeDeleteMany(model, table)
  }

  try {
    // Crear perfil por defecto solo si la tabla existe
    if (await tableExists('Profile')) {
