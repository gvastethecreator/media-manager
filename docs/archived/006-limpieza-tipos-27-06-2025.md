...
# [006] Limpieza y Organización de Tipos - COMPLETADO

## Context

✅ **COMPLETADO:** La auditoría y limpieza completa de tipos ha sido finalizada exitosamente.

La carpeta `src/types` ha sido auditada, limpiada y reorganizada, eliminando archivos duplicados, obsoletos y sin uso. Se han consolidado los tipos y se ha establecido una estructura clara y moderna.

## Subtasks

- [x] [HIGH] [MEDIUM] Auditar carpeta `src/types` - identificar duplicados y archivos obsoletos
- [x] [HIGH] [SMALL] Eliminar tipos sin uso o duplicados
- [x] [MEDIUM] [MEDIUM] Mover tipos a sus carpetas correspondientes según entidad
- [x] [MEDIUM] [SMALL] Consolidar archivos de tipos relacionados
- [x] [LOW] [SMALL] Actualizar importaciones que apunten a archivos movidos
...
### 🧹 Limpieza de Archivos Obsoletos y Duplicados

- ✅ **Archivos legacy eliminados**:
  - `src/types/entities/character-export.ts` (deprecated)
  - `src/types/entities/file-3d/` (duplicada de `file3d/`)
  - `src/types/entities/metadata/base.ts` y `src/types/entities/note/base.ts` (deprecated)
  - `src/types/queue.ts` (duplicado de `entities/queue-job/types.ts`)
  - `src/types/entities/collection/types.ts` (causaba errores)
  - `src/types/entities/imagestats/` (carpeta sin uso)

- ✅ **Archivos de formularios sin uso eliminados**:
  - `src/types/album-form-data.ts`
  - `src/types/character-form-data.ts`
  - `src/types/collection-form-data.ts`
  - `src/types/group.types.ts`
  - `src/types/image-item.ts`
...
- ✅ **Archivos de tipos generales sin uso**:
...
### 🔄 Reorganización de Tipos de UI

- ✅ **Separación de responsabilidades**:
  - `src/types/entities/settings/` → `src/types/ui/` (tipos específicos de interfaz)
  - `src/types/settings.ts` → Mantenido como fuente canónica para configuración global
  - Clara separación entre tipos de BD y tipos de UI

- ✅ **Actualización de importaciones**:
  - `src/store/entities/settings/store.ts` → Actualizado para usar `@/types/ui/`
  - `src/lib/types.ts` → Documentación actualizada con nuevas rutas

### � Consolidación de Metadatos

- ✅ **Eliminación de duplicación**:
  - `src/types/metadata.types.ts` eliminado
  - `src/types/metadata.ts` consolidado con tipos completos
  - Importaciones migradas exitosamente

- ✅ **Tipos mantenidos por uso activo**:
  - `src/types/metadata.ts` y `src/types/entities/metadata/` (diferentes propósitos)
  - `src/types/uploaded-images.ts` y `src/types/entities/uploaded-image/` (diferentes propósitos)
  - `src/types/settings.ts` (configuración global)

### 🧹 Migración de Importaciones

- ✅ **Migraciones exitosas**:
  - `src/hooks/use-group-sort.ts` → `@/types/entities/group`
  - `src/components/cards/collection-card/collection-server-actions.ts` → Tipos canónicos
  - `src/store/entities/settings/store.ts` → `@/types/ui/`
  - Todas las importaciones de `metadata.types.ts` → `metadata.ts`

### 🔍 Verificación y Validación

- ✅ **Errores de TypeScript corregidos**: Múltiples verificaciones con `pnpm tsc --noEmit --skipLibCheck`
- ✅ **Importaciones actualizadas**: Todas las referencias apuntan a archivos existentes
- ✅ **Estructura limpia**: Solo archivos necesarios y con uso activo
- ✅ **Separación clara**: Tipos de BD, tipos de UI, tipos de dominio bien organizados

## Progress

✅ **Fase 1 - Refactorización de Entidades:** COMPLETADA
✅ **Fase 2 - Limpieza de Tipos:** COMPLETADA

## Resultado Final

La carpeta `src/types` ahora cuenta con:

- **Estructura organizada** por propósito y entidad
- **Sin duplicación** de tipos o archivos
- **Separación clara** entre tipos de BD (`entities/`), tipos de UI (`ui/`), y tipos de dominio (raíz)
- **Importaciones actualizadas** y funcionando correctamente
- **Verificación TypeScript** exitosa sin errores

Esta limpieza mejora significativamente la mantenibilidad del código y reduce la confusión entre tipos similares.
