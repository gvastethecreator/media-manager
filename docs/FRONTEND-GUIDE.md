...
const CharactersView = lazy(() =>
  import('@/components/views/characters/characters-view')
    .then(m => ({ default: m.CharactersView }))
);
...
function Gallery({ images }) {
  ...
  return images.map(image => (
    <ImageCard key={image.id} image={image} onClick={() => handleClick(image)} />
  ));
}
```
...
export const MyComponent = memo(forwardRef<HTMLDivElement, MyComponentProps>(
