...
describe('04-services-and-layers', () => {
	...
	describe('Layer Implementation', () => {
		...
		it.effect('composes layers with dependencies', () =>
			Effect.gen(function* () {
				class Logger extends ServiceMap.Service<Logger, { readonly log: (msg: string) => Effect.Effect<void> }>()(
					'@app/Logger'
				) {}
	...
	describe('Effect.fn within Services', () => {
		...
		it.effect('supports Effect.fn transformers (e.g., catchTag)', () =>
			Effect.gen(function* () {
				...
				class Repo extends ServiceMap.Service<Repo, { readonly findById: (id: string) => Effect.Effect<string> }>()(
					'@app/Repo'
				) {
	...
	describe('Service Composition', () => {
		it.effect('merges independent layers', () =>
			Effect.gen(function* () {
				class ServiceA extends ServiceMap.Service<ServiceA, { readonly getValue: () => Effect.Effect<string> }>()(
					'@app/ServiceA'
				) {}

				class ServiceB extends ServiceMap.Service<ServiceB, { readonly getValue: () => Effect.Effect<number> }>()(
					'@app/ServiceB'
				) {}
		...
		it.effect('Layer.provide satisfies dependencies', () =>
			Effect.gen(function* () {
				...
				class Database extends ServiceMap.Service<Database, { readonly query: () => Effect.Effect<string> }>()(
					'@app/Database'
				) {
					static readonly layer = Layer.effect(
						Database,
		...
		it.effect('Layer.provideMerge exposes the provider', () =>
			Effect.gen(function* () {
				...
				class Logger extends ServiceMap.Service<Logger, { readonly log: () => Effect.Effect<string> }>()(
					'@app/Logger'
				) {
					static readonly layer = Layer.effect(
						Logger,
	...
	describe('Providing Layers to Effects', () => {
		it.effect('provide once at entry point', () =>
			Effect.gen(function* () {
				...
				class Logger extends ServiceMap.Service<Logger, { readonly info: (msg: string) => Effect.Effect<void> }>()(
					'@app/Logger'
				) {}

				class App extends ServiceMap.Service<App, { readonly run: () => Effect.Effect<string> }>()('@app/App') {
					static readonly layer = Layer.effect(
						App,
						Effect.gen(function* () {
							const config = yield* Config;
	...
	describe('Layer Memoization', () => {
		it.effect('memoizes layers by reference identity', () =>
			Effect.gen(function* () {
				...
				class ServiceA extends ServiceMap.Service<ServiceA, { readonly getValue: () => Effect.Effect<number> }>()(
					'@app/ServiceA'
				) {}
