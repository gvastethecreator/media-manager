...
const release = Command.make('release').pipe(
	Command.withDescription('Generate assets, version packages, and push release tags'),
	Command.withHandler(() =>
		Effect.gen(function* () {
			...
			// Push each tag explicitly so remotes receive every release tag.
			yield* Effect.log(`Pushing ${tags.length} tag(s)...`);
			for (const tag of tags) {
				yield* exec(`git push origin ${tag}`);
			}

			yield* Effect.log(`Release tags pushed: ${tags.join(', ')}`);
		})
	)
);
