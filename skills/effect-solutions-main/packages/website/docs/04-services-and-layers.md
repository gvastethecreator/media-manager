...
class UserNotFoundError extends Schema.TaggedErrorClass('UserNotFoundError')('UserNotFoundError', {
	id: UserId,
}) {}

class GenericUsersError extends Schema.TaggedErrorClass('GenericUsersError')('GenericUsersError', {
	id: UserId,
	error: Schema.Defect,
}) {}
		...
		Effect.gen(function* () {
			...
			const findById = Effect.fn('Users.findById')(
				...
				Effect.catchTag('ResponseError', (error) =>
					error.response.status === 404 ? new UserNotFoundError({ id }) : new GenericUsersError({ id, error })
				)
			);
...
class UserService extends ServiceMap.Service<UserService, { readonly getUser: () => Effect.Effect<void> }>()(
	'@app/UserService'
) {}
...
// hide-start
import { Effect, ServiceMap } from 'effect';
class SqlClient extends ServiceMap.Service<SqlClient, { readonly query: (sql: string) => Effect.Effect<unknown[]> }>()(
	'@app/SqlClient'
) {}
class Postgres {
...
const badAppLayer = Layer.merge(
	UserRepo.layer.pipe(Layer.provide(Postgres.layer({ url: 'postgres://localhost/mydb', poolSize: 10 }))),
