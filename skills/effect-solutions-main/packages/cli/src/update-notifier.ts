  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const isNewer = (latest: string, current: string): boolean => {
        const parseParts = (v: string) => v.split(".").map((x) => Number.parseInt(x, 10) || 0)

        const [latestMajor = 0, latestMinor = 0, latestPatch = 0] = parseParts(latest)
        const [currentMajor = 0, currentMinor = 0, currentPatch = 0] = parseParts(current)
      ...
      const cachePath = (pkgName: string) => path.join(config.homeDir, ".config", pkgName, "update.json")
      ...
      const writeCache = Effect.fn("UpdateNotifier.writeCache")((file: string, data: CacheFile) =>
        Effect.tryPromise({
          try: async () => {
      ...
      const logUpdate = Effect.fn("UpdateNotifier.logUpdate")((current: string, latest: string, pkgName: string) =>
        Console.log(
          [`Update available for ${pkgName}: ${current} → ${latest}`, `Run: bun add -g ${pkgName}@latest`].join("\n"),
        ),
      )

      const check = Effect.fn("UpdateNotifier.check")((pkgName: string, currentVersion: string) =>
        Effect.gen(function* () {
