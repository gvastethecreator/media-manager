...
describe('maybeNotifyUpdate (effect)', () => {
	...
	const cachePath = (home: string) => path.join(home, '.config', pkgName, 'update.json');
	...
	test('emits hint when cache says newer version', async () => {
		...
		await Effect.runPromise(program.pipe(Effect.provide(UpdateNotifier.layer), Effect.provide(testConfigLayer)));
