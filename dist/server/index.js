...
var CreateFolderSchema = external_exports.object({
  name: external_exports.string().min(1, "El nombre es requerido").max(255),
  description: external_exports.string().max(1e3).optional().nullable(),
  path: external_exports.string().min(1, "La ruta es requerida").max(500),
  emoji: external_exports.string().max(10).optional().nullable(),
  color: external_exports.string().regex(/^#[0-9A-Fa-f]{6}$/).optional().nullable(),
  featuredImage: external_exports.string().url().optional().nullable(),
  isFavorite: external_exports.boolean().default(false).optional(),
  autoReindex: external_exports.boolean().default(true),
  parentId: external_exports.string().uuid().optional().nullable(),
  presetId: external_exports.string().uuid().optional().nullable()
});
var UpdateFolderSchema = CreateFolderSchema.partial().omit({ path: true });
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    if (!external_exports.string().uuid().safeParse(id).success) {
      return res.status(400).json({ error: "ID de carpeta inv\xE1lido" });
    }
  ...
  try {
    ...
    const updatedFolder = await prisma.folder.update({
      ...
      data: {
        ...
        ...data.emoji !== void 0 && { emoji: data.emoji },
        ...data.color !== void 0 && { color: data.color },
        ...data.featuredImage !== void 0 && { featuredImage: data.featuredImage },
        ...data.isFavorite !== void 0 && { isFavorite: data.isFavorite },
        ...data.parentId !== void 0 && { parentId: data.parentId },
        ...data.presetId !== void 0 && { presetId: data.presetId }
      },
      include: {
        _count: {
          select: {
...
app.listen(PORT, () => {
  console.log(`\u{1F680} Express server running on http://localhost:${PORT}`);
  console.log(`\u{1F4CA} Health check: http://localhost:${PORT}/api/health`);
  console.log(`\u{1F4C1} Folders API: http://localhost:${PORT}/api/folders`);
});
