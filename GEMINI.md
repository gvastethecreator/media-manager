...
| :--- | :--- | :--- |
| **Frontend** | React 19, Vite 7 | Zustand, TanStack Query, Tailwind CSS 4, Radix UI |
...
*   **Linting:** Use **Biome**. `bun run biome:fix` and `bun run format` are your friends.
...
1.  **Service Layer (`src/services/<entity>/`)**
    ...
    *   **Rule:** Controllers/Routes should *never* query Drizzle directly. They must use Services.
...
*   **Bun Issues**: If a package fails, check if it has a Bun-compatible alternative.
